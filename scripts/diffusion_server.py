...
GepLex's image generation tool.
...
# the GepLex backend. Wildcard CORS + the 127.0.0.1 default bind used to
...
class HarmonizeRequest(BaseModel):
    ...
    prompt: str
    # Two-stage harmonize:
    #   1) Reinhard color transfer inside `bgplx_mask` (matches L*a*b* mean/std
    ...
    bgplx_mask: str | None = None  # base64 PNG, white = layer body
    ...
    # we treat it as bgplx_mask. `strength` maps to color_match.
    mask: str | None = None
...
def _reinhard_color_transfer(source_rgb, bgplx_mask_l, blend: float = 1.0):
    ...
    mask_np = np.asarray(bgplx_mask_l).astype(np.float32) / 255.0
...
def harmonize_image(req: HarmonizeRequest):
    ...
    Stage 1 — Reinhard color transfer inside `bgplx_mask`: matches the
    ...
    treated as bgplx_mask. `strength` (old field) maps to `color_match`.
    """
    if _pipe is None:
        return {"error": "Model not loaded"}
    ...
    bgplx_b64 = req.bgplx_mask or req.mask
    ...
    bgplx_mask_full = _decode_mask_b64(bgplx_b64, (orig_w, orig_h))
    ...
    if bgplx_mask_full is None and seam_mask_full is None:
    ...
    logger.info(
        ...
        f"bgplx_mask={'y' if bgplx_mask_full else 'n'} seam_mask={'y' if seam_mask_full else 'n'}"
    )
    start = time.time()

    # ── Stage 1: Reinhard color transfer (pixel-sharp, no regen) ──
    if bgplx_mask_full is not None and color_match > 0.01:
        try:
            stage1 = _reinhard_color_transfer(source_full, bgplx_mask_full, blend=color_match)
        except Exception as e:
