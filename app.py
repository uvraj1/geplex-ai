...
from core.constants import (
    BASE_DIR, STATIC_DIR, ASSETS_DIR, SESSIONS_FILE,
...
try:
    ...
    # GepLex is single-process by convention, so this is acceptable, but be aware that
...
app = FastAPI(
    title="GepLex",
    description="GepLex AI workspace for chat, research, documents, notes, and local model workflows",
    version="1.0.0",
)
...
_TIMEOUT_EXEMPT_PREFIXES = (
    ...
    "/api/admin/learning-ai", # teacher test/learning can exceed the 45s fallback
)
...
class _SlowRequestLogMiddleware(_BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        try:
            ...
            try:
                threshold = float(os.getenv("GEPLEX_SLOW_REQUEST_LOG_SECONDS", "0.75") or "0.75")
            except Exception:
                threshold = 0.75
...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. GepLex's own
    ...
    class AuthMiddleware(BaseHTTPMiddleware):
        async def dispatch(self, request: Request, call_next):
            ...
            if _is_auth_exempt(path):
                return await call_next(request)
            # The bundled GepLex Local AI endpoint is called over HTTP by the
            # same process. It has no provider API key; permit only direct
            # loopback access so stale Ollama bearer headers cannot cause 401s.
            if path.startswith("/api/local-model/") and _is_trusted_loopback(request):
                return await call_next(request)
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-GepLex-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-GepLex-Owner") or "").strip()
            ...
            if auth_header.startswith("Bearer gplx_"):
                raw_token = auth_header[7:]
                # Sanity check: tokens are "gplx_" + 43 chars of base64
...
# Bundled OpenAI-compatible backend. It is protected by the same in-process
# token used by server-side tool loopback calls.
from routes.local_model_routes import setup_local_model_routes
app.include_router(setup_local_model_routes())
...
class _RevalidatingStatic(StaticFiles):
    ...
    return a cheap 304 (ETag/Last-Modified are preserved). Brand assets
    (.svg favicons, wordmarks) also use no-cache so logo changes show up
    immediately for all connected browsers."""

    async def get_response(self, path, scope):
        resp = await super().get_response(path, scope)
        if path.endswith((".js", ".css", ".html", ".svg", ".ico", ".png", ".json")):
            resp.headers["Cache-Control"] = "no-cache"
        return resp


app.mount("/static", _RevalidatingStatic(directory=STATIC_DIR), name="static")
app.mount("/assets", _RevalidatingStatic(directory=ASSETS_DIR), name="assets")
...
async def serve_favicon():
    icon_path = abs_join(BASE_DIR, "static", "icon.ico")
    if not os.path.exists(icon_path):
        icon_path = abs_join(BASE_DIR, "static", "favicon.ico")
    return FileResponse(
        str(icon_path),
...
async def serve_apple_touch_icon():
    icon_path = abs_join(BASE_DIR, "static", "icons", "icon-192.png")
    return FileResponse(
        str(icon_path),
...
async def _startup_event():
    ...
    if sys.platform == "win32":
        ...
        def _windows_asyncio_exception_handler(current_loop, context):
            exception = context.get("exception")
            handle = context.get("handle")
            if (
                isinstance(exception, ConnectionResetError)
                and handle is not None
                and "_ProactorBasePipeTransport._call_connection_lost" in repr(handle)
            ):
                logger.debug("Ignored expected Windows client disconnect during pipe cleanup")
                return
    ...
    try:
        ...
        logger.info("Ensured default model seed: %s (%s)", settings.get("default_model"), settings.get("default_endpoint_id"))
    except Exception as exc:
        logger.warning("Default-model seed check failed: %s", exc)
    ...
    _startup_warmups_enabled = str(os.getenv("GEPLEX_STARTUP_WARMUPS", "")).lower() in {"1", "true", "yes", "on"}
    if _startup_warmups_enabled:
        ...
        logger.info("Startup warmups disabled (set GEPLEX_STARTUP_WARMUPS=1 to enable)")
    ...
    _keepalive_enabled = str(os.getenv("GEPLEX_MODEL_KEEPALIVE", "")).lower() in {"1", "true", "yes", "on"}
    ...
    # `GEPLEX_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("GEPLEX_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (GEPLEX_INPROCESS_TASKS=0); "
...
if __name__ == "__main__":
    ...
    bind_host = os.getenv("APP_BIND", "0.0.0.0" if os.getenv("PORT") else "127.0.0.1")
    bind_port = int(os.getenv("PORT", os.getenv("APP_PORT", "7000")))
