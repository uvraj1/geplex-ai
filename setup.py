#!/usr/bin/env python3
"""GepLex — first-time setup script.
...
def create_default_admin():
    ...
    try:
        ...
        username = os.getenv("GEPLEX_ADMIN_USER", "").strip().lower()
        password = os.getenv("GEPLEX_ADMIN_PASSWORD", "").strip()

        if username and password:
            # Both provided via env — validate before using
            if username in RESERVED_USERNAMES:
                print(f"  [error] GEPLEX_ADMIN_USER '{username}' is a reserved username")
                return "failed"
            if len(password) < PASSWORD_MIN_LENGTH:
                print(f"  [error] GEPLEX_ADMIN_PASSWORD must be at least {PASSWORD_MIN_LENGTH} characters")
                return "failed"
        elif sys.stdin.isatty() and not os.getenv("GEPLEX_SKIP_ADMIN_PROMPT"):
        ...
        if sys.stdin.isatty() and not os.getenv("GEPLEX_ADMIN_PASSWORD"):
            print(f"  [ok] Admin account created ({username})")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not os.getenv("GEPLEX_ADMIN_PASSWORD"):
                print(f"        Temporary password: {password}")
                print(f"        ** Change it after first login. Set GEPLEX_ADMIN_PASSWORD to choose your own. **")
        return "created"
...
def seed_default_model():
    """Persist the GepLex default model and endpoint if unset."""
    try:
        from src.settings import ensure_default_model_seed
        settings = ensure_default_model_seed()
        print(f"  [ok] Default AI model seeded: {settings.get('default_model')} ({settings.get('default_endpoint_id')})")
        return True
    except Exception as exc:
        print(f"  [warn] Default AI model seeding failed: {exc}")
        return False
...
def main():
    ...
    # Load .env so pre-seeded GEPLEX_ADMIN_USER / GEPLEX_ADMIN_PASSWORD (and
    ...
    if not os.getenv("GEPLEX_SKIP_RUN_HINT"):
