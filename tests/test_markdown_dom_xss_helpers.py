...
def test_email_rich_bgplx_render_path_reuses_raw_html_sanitizer():
    ...
    email_bgplx_helper = document_src.split("function _emailBodyToHtml(text)", 1)[1].split(
    ...
    assert "markdownModule.sanitizeAllowedHtml(t)" in email_bgplx_helper
    assert "return t;" not in email_bgplx_helper
