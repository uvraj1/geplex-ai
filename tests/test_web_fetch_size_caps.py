...
def test_bgplx_under_cap_is_untouched(monkeypatch, no_cache):
...
def test_bgplx_over_soft_cap_truncates_with_flags(monkeypatch, no_cache):
...
def test_override_is_clamped_to_hard_cap(monkeypatch, no_cache):
    ...
    assert big.bgplx_reads == 0  # refused before buffering
...
def test_rejects_compressed_response_that_ignored_identity(monkeypatch, no_cache):
    ...
    assert fake.bgplx_reads == 0  # refused before decoding any body
