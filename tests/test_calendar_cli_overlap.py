"""Regression: `geplex-calendar list` must select events that OVERLAP the
...
`geplex-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "geplex-calendar"
    loader = importlib.machinery.SourceFileLoader("geplex_calendar_cli", str(path))
