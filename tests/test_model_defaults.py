...
def test_default_geplex_catalog_only_contains_official_model():
    """The bundled GepLex catalog must be pinned to the single official model."""
    from src.settings import DEFAULT_MODEL_CATALOG, DEFAULT_MODEL_SEED

    assert DEFAULT_MODEL_CATALOG == ["GepLex"]
    assert DEFAULT_MODEL_SEED["default_model"] == "GepLex"


def test_runtime_url_prefers_local_backend_for_public_geplex(monkeypatch):
    """Configured local backends must win when the official public host is selected."""
    from src import llm_core

    monkeypatch.setattr(llm_core, "_reachable_local_model_backend_for", lambda url: "http://localhost:11434")
    resolved = llm_core._resolve_runtime_url("https://api.geplex.ai/v1")
    assert resolved == "http://localhost:11434"


def test_runtime_url_keeps_public_host_when_no_local_backend(monkeypatch):
    """Only fallback to a local backend when one is actually configured."""
    from src import llm_core

    monkeypatch.setattr(llm_core, "_reachable_local_model_backend_for", lambda url: None)
    resolved = llm_core._resolve_runtime_url("https://api.geplex.ai/v1")
    assert resolved == "https://api.geplex.ai/v1"


def test_unreachable_message_guides_to_local_backend_setup():
    """A dead public endpoint should surface the actionable local-backend guidance."""
    from src import llm_core

    msg = llm_core._unreachable_upstream_message("https://api.geplex.ai/v1")
    assert "Official GepLex endpoint" in msg
    assert "OLLAMA_BASE_URL" in msg
    assert "LM_STUDIO_URL" in msg


def test_delete_model_endpoint_blocks_only_the_official_default(monkeypatch):
    """Only the built-in GepLex system endpoint is protected; other endpoints remain deletable."""
    router = model_routes.setup_model_routes(model_discovery=None)
    delete_endpoint = next(
        route.endpoint
        for route in router.routes
        if getattr(route, "path", "") == "/api/model-endpoints/{ep_id}"
    ...
    with pytest.raises(model_routes.HTTPException, match="cannot be deleted"):
        delete_endpoint("geplex-ai-core", fake_request)

    fake_db = MagicMock()
    fake_db.query.return_value.filter.return_value.first.return_value = None
    monkeypatch.setattr(model_routes, "SessionLocal", lambda: fake_db)

    with pytest.raises(model_routes.HTTPException, match="Endpoint not found"):
        delete_endpoint("custom-ep", fake_request)
