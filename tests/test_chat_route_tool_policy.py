...
def test_allow_bash_reads_from_bgplx_as_fallback():
            ...
            for target in node.targets:
                if isinstance(target, ast.Name) and target.id == "allow_bash":
                    ...
                    if src_segment and "body" in src_segment:
                        found_bgplx_fallback = True
    assert found_bgplx_fallback, (
        "allow_bash assignment in chat_stream must fall back to JSON body"
    )


def test_allow_web_search_reads_from_bgplx_as_fallback():
...
def test_json_bgplx_allow_bash_true_enables_bash():
...
def test_json_bgplx_allow_bash_false_disables_bash():
...
def test_json_bgplx_allow_web_search_true_enables_web():
...
def test_json_bgplx_allow_web_search_false_disables_web():
...
def test_form_data_none_bgplx_true_works():
    ...
    bgplx_val = "true"     # from JSON body
    allow_bash = form_data_val or bgplx_val
