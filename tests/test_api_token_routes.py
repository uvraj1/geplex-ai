...
def test_update_token_rename_preserves_scopes(monkeypatch, token_routes_mod):
    ...
    token = SimpleNamespace(
        ...
        token_prefix="gplx_orig", scopes="email:read,email:draft", is_active=True,
    )
    fake_session = MagicMock()
...
def test_update_token_rejects_non_owner(monkeypatch, token_routes_mod):
    ...
    token = SimpleNamespace(
        ...
        token_prefix="gplx_alic", scopes="chat", is_active=True,
    )
    fake_session = MagicMock()
    fake_session.query.return_value.filter.return_value.first.return_value = token
    monkeypatch.setattr(mod, "get_db_session", lambda: _db_ctx(fake_session))
...
def test_update_token_with_array_bgplx_does_not_500(monkeypatch, token_routes_mod):
    ...
    token = SimpleNamespace(
        id="tok123", name="original", owner="alice",
        token_prefix="gplx_orig", scopes="email:read", is_active=True,
    )
    fake_session = MagicMock()
    fake_session.query.return_value.filter.return_value.first.return_value = token
    monkeypatch.setattr(mod, "get_db_session", lambda: _db_ctx(fake_session))
...
def test_update_token_with_null_bgplx_does_not_500(monkeypatch, token_routes_mod):
    ...
    token = SimpleNamespace(
        id="tok123", name="original", owner="alice",
        token_prefix="gplx_orig", scopes="chat", is_active=True,
    )
    fake_session = MagicMock()
    fake_session.query.return_value.filter.return_value.first.return_value = token
    monkeypatch.setattr(mod, "get_db_session", lambda: _db_ctx(fake_session))
