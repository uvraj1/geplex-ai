...
def test_ollama_cookbook_runner_does_not_force_public_bind():
    ...
    assert 'OLLAMA_HOST="0.0.0.0:${GEPLEX_OLLAMA_PORT}" ollama serve' not in route
    assert 'OLLAMA_HOST="${GEPLEX_OLLAMA_HOST}:${GEPLEX_OLLAMA_PORT}" ollama serve' in route
...
def test_inprocess_pollers_gate(monkeypatch):
    """The GEPLEX_INPROCESS_POLLERS env var must let operators kill
    the asyncio pollers when cron / systemd is driving the one-shot
    `geplex-mail poll-*` CLI subcommands instead. Two pollers racing
    ...
    for on in ("1", "true", "yes", "anything-truthy"):
        monkeypatch.setenv("GEPLEX_INPROCESS_POLLERS", on)
...
def _load_search_content_for_test(monkeypatch, name="services.search.content_under_test"):
    ...
    cache.CONTENT_CACHE_DIR = Path("/tmp/geplex-test-content-cache")
...
def test_email_thread_rendering_sanitizes_bgplx_html():
    """Both threaded render paths must run server-parsed bgplx_html through the
    allowlist sanitizer (the flat path already did)."""
    src = Path(__file__).resolve().parents[1] / "static" / "js" / "emailLibrary.js"
    text = src.read_text()
    # every `t.bgplx_html` reference is wrapped by _sanitizeHtml(...)
    assert text.count("t.bgplx_html") == text.count("_sanitizeHtml(t.bgplx_html")
    assert "t.bgplx_html" in text  # guard against the file being refactored away
...
def test_google_mcp_oauth_uses_configured_redirect_base(monkeypatch):
    monkeypatch.setenv("OAUTH_REDIRECT_BASE_URL", "https://geplex.example/app/")
    ...
    assert (
        ...
        == "https://geplex.example/app/api/mcp/oauth/callback"
    )
