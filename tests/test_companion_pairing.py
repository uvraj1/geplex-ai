...
def test_mint_pairing_token_invalidates_cache(monkeypatch):
    ...
    monkeypatch.setattr(P, "mint_token", lambda owner, name="companion": ("id1", "gplx_demo"))
    invalidate = MagicMock()
    token_id, raw = mint_pairing_token("alice", invalidate)
    assert (token_id, raw) == ("id1", "gplx_demo")
...
def test_mint_pairing_token_tolerates_no_invalidator(monkeypatch):
    ...
    assert mint_pairing_token("alice", None) == ("id1", "gplx_demo")


def test_pairing_payload_shape():
    p = P.pairing_payload("192.168.1.9", 7000, "gplx_x")
    assert p == {"v": 1, "host": "192.168.1.9", "port": 7000, "token": "gplx_x"}


@pytest.mark.parametrize(
    ("value", "expected"),
    [
        ("http://geplex", ("geplex", 80)),
        ("http://geplex:7000", ("geplex", 7000)),
        ("http://localhost:7000", ("localhost", 7000)),
        ("http://geplex.local", ("geplex.local", 80)),
        ("http://api.geplex.local:7000", ("api.geplex.local", 7000)),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "ftp://geplex.example",
        "https://geplex.local",
        "http://user:password@geplex.local",
        "http://geplex.local/",
        "http://geplex.local/path",
        "http://geplex.local?query=1",
        "http://geplex.local#fragment",
        "http://geplex.local:not-a-port",
        "http://geplex.local:0",
        "http://geplex.local:65536",
        "http://geplex.local:07000",
        "HTTP://geplex.local:7000",
        "http://GepLex.local:7000",
        " http://geplex.local",
        "http://geplex.local ",
        "http://geplex\\local",
        ...
        "http://geplex%2elocal:7000",
        "http://%31%39%32.168.1.9:7000",
        "http://geplex%40local:7000",
        "http://.local:7000",
        "http://geplex..local:7000",
        "http://geplex.local.:7000",
        "http://-geplex:7000",
        "http://geplex-:7000",
        "http://geplex_name:7000",
        f"http://{'a' * 64}:7000",
...
def test_pair_post_json_returns_pairing_payload(monkeypatch):
    mint = MagicMock(return_value=("tok123", "gplx_raw"))
    monkeypatch.setattr(R, "require_admin", lambda request: None, raising=False)
    ...
    assert response["token"] == "gplx_raw"
...
def test_pair_post_json_prefers_configured_origin(monkeypatch):
    monkeypatch.setenv("COMPANION_BASE_URL", "http://geplex.local:7000")
    mint = MagicMock(return_value=("tok123", "gplx_raw"))
    ...
    assert response["host"] == "geplex.local"
    assert response["port"] == 7000
    assert response["hosts"] == ["geplex.local"]
    assert set(response) == {
        "host",
        "port",
        "token",
...
def test_pair_post_rejects_invalid_config_before_mint_without_echoing_it(monkeypatch):
    ...
    monkeypatch.setenv(
        "COMPANION_BASE_URL",
        f"http://admin:{configured_secret}@geplex.local",
    )
...
def test_pair_post_json_qr_failure_returns_null_qr(monkeypatch):
    monkeypatch.setattr(R, "require_admin", lambda request: None, raising=False)
    monkeypatch.setattr(R, "get_current_user", lambda request: "alice")
    monkeypatch.setattr(R, "mint_pairing_token", lambda owner, invalidate: ("tok123", "gplx_raw"))
    monkeypatch.setattr(R._pairing, "lan_ip_candidates", lambda: ["192.168.1.50"])
    monkeypatch.setattr(R._pairing, "pairing_qr_png_data_uri", lambda payload: None)
...
def test_pair_post_html_escapes_pairing_values(monkeypatch):
    monkeypatch.setattr(R, "require_admin", lambda request: None, raising=False)
    monkeypatch.setattr(R, "get_current_user", lambda request: "alice")
    monkeypatch.setattr(R, "mint_pairing_token", lambda owner, invalidate: ("tok<123>", "gplx_<raw>&"))
    ...
    assert "gplx_<raw>&" not in body
    assert "tok<123>" not in body
    assert "host&lt;one&gt;&amp;" in body
    assert "gplx_&lt;raw&gt;&amp;" in body
