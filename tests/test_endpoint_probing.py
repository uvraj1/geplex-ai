...
probe surface that drives endpoint setup and degraded-state reporting:
  ...
  * `_ping_endpoint`      — reachability classification: 2xx, auth failures,
    the "this is GepLex, not a model server" /login-redirect trap, generic
...
class TestProbeEndpointParsing:
    ...
    def test_non_dict_json_bgplx_degrades_to_empty(self, monkeypatch, caplog, body):
...
class TestPingEndpoint:
    ...
    def test_detects_geplex_login_redirect(self, monkeypatch):
        _patch_resolve(monkeypatch)
