...
def test_pdf_bgplx_marker_stripped_without_eating_text(monkeypatch, tmp_path):
    ...
    bgplx_lines = body.splitlines()
    # The leading page marker and page text must survive intact.
    assert "[Page 1 text]:" in bgplx_lines
    assert "to the board, the agenda is set" in bgplx_lines
    # The old lstrip(chars) corruption produced a line like "age 1 text]:" (missing "[P").
    assert "age 1 text]:" not in bgplx_lines
