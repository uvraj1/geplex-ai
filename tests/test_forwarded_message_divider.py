...
English "Forwarded message" one, so forwarded mail produced by GepLex itself
...
def test_forwarded_divider_not_leaked_into_reply_body():
    ...
    assert "Forwarded message" not in turns[0]["bgplx_html"]
    # No turn at all should carry the raw divider in its rendered body.
    assert all("Forwarded message" not in t["bgplx_html"] for t in turns)
    ...
    assert "Forwarded body content." in deeper[0]["bgplx_html"]
