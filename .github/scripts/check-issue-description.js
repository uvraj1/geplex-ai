...
module.exports = async ({ github, context, core }) => {
  ...
  switch (type) {
    ...
    case 'bug': {
      const revisionText = section('GepLex Revision');
      if (!/^[0-9a-f]{12} \(\d{4}-\d{2}-\d{2}\)$/i.test(revisionText)) {
        failures.push(
          '**GepLex Revision** — paste the 12-character commit SHA and date, ' +
          'for example `1fef4929cf1d (2026-08-11)`',
        );
      }

      if (!section('Install Method')) {
        failures.push('**Install Method** — select how you installed GepLex');
      }
    ...
    // 'untyped' → only the common bgplx-length check applies.
  }
