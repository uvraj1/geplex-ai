    ...
    if channel == "email":
        try:
            ...
            if missing:
                ...
                msg["Subject"] = f"Reminder (GepLex): {_t}"
                msg["Date"] = _dt.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
                msg["X-GepLex-Origin"] = "geplex-ui"
                msg["X-GepLex-Kind"] = "reminder"
                msg["X-GepLex-Ref"] = str(note_id)
                ...
                _bgplx_chunks = []
                if synthesis:
                    _bgplx_chunks.append(synthesis)
                if _t:
                    _bgplx_chunks.append(_t)
                if note_body:
                    _bgplx_chunks.append(note_body)
                plain = "\n\n".join(_bgplx_chunks) if _bgplx_chunks else title
