...
def build_uploaded_file_manifest(att_ids: list, upload_handler, owner: Optional[str]) -> list[dict]:
    ...
    for att_id in att_ids:
        ...
        ref.update({
            ...
            "uri": f"geplex://attachment/{ref['attachment_id']}",
...
def resolve_session_auth(sess, session_id: str, owner: Optional[str] = None):
    ...
    is_local_model = "/api/local-model/" in (getattr(sess, "endpoint_url", "") or "")
    if has_auth and not is_chatgpt_subscription and not is_local_model:
        return
        ...
        try:
            ...
            for ep in q.all():
                ...
                try:
                    ...
                    return
                if not api_key:
                    if "/api/local-model/" in base:
                        sess.headers = build_internal_loopback_headers(owner)
                    else:
...
def run_post_response_tasks(
    ...
    *,
    ...
    # Memory extraction runs every second completed turn. The extractor itself
    # is conservative (max two durable facts) and the sequential queue prevents
    # this side call from competing with the user's next completion.
    _msg_count = len(sess.history) if hasattr(sess, 'history') else 0
    _should_extract = (_msg_count >= 2) and (_msg_count % 2 == 0)
