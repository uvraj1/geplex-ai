...
from src.auth_helpers import effective_user, _auth_disabled, owner_filter, is_admin_request
...
def _verify_session_owner(request: Request, session_id: str, session_manager=None):
    ...
    if is_admin_request(request, user):
        return
    db = SessionLocal()
    try:
...
def setup_session_routes(
    ...
    def list_sessions(request: Request):
        user = effective_user(request)
        # Administrators may audit every tenant; regular users get only their
        # own sessions. This scope is applied to both the in-memory and SQL
        # sources so the sidebar cannot combine data from different users.
        is_admin = is_admin_request(request, user)
        query_user = None if is_admin else user
        ...
        try:
            ...
            pass
        user_sessions = session_manager.get_sessions_for_user(query_user)
        ...
        try:
            ...
            q = owner_filter(q, DbSession, query_user)
            rows = q.all()
            for row in rows:
