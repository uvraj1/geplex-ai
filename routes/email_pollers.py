                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in GepLex (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in GepLex" button
                                    import html as _h
                                    bgplx_excerpt = _h.escape((body or "")[:800])
                                    open_html = (
                                    ...
                                    alert_html = (
                                        ...
                                        f'</p>'
                                        f'<pre style="white-space:pre-wrap;font-family:inherit;background:#f6f8fa;padding:10px;border-radius:4px;font-size:13px">{bgplx_excerpt}'
                                        + ("..." if len(body or "") > 800 else "")
                                        + "</pre></div>"
                                    )
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "geplex_kind" if "geplex_kind" in cols else "'scheduled' AS geplex_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                # externally cron-driven `geplex-mail poll-scheduled`, or
                # an admin running the CLI manually alongside the in-process
                # one despite the GEPLEX_INPROCESS_POLLERS=0 guidance) -
                ...
                geplex_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-GepLex-Origin"] = "geplex-ui"
                outer["X-GepLex-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", geplex_kind or "scheduled")[:64]
                outer["X-GepLex-Ref"] = sid
                if r[6]:
                    outer["In-Reply-To"] = r[6]
                if r[7]:
                    outer["References"] = r[7]
                bgplx_container.attach(MIMEText(r[5] or "", "plain", "utf-8"))
                html_body = html.escape(r[5] or "").replace("\n", "<br>\n")
                bgplx_container.attach(MIMEText(f"<html><body>{html_body}</body></html>", "html", "utf-8"))
                if has_atts:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `geplex-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `GEPLEX_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `geplex-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("GEPLEX_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `GEPLEX_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (GEPLEX_INPROCESS_POLLERS=0); "
            "drive `geplex-mail poll-scheduled` externally."
        )
        return
    import asyncio
