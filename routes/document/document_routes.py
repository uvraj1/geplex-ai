...
from src.auth_helpers import get_current_user, _auth_disabled, is_admin_request
...
def _get_session_or_404(db, session_id: str, user: Optional[str], *, allow_admin: bool = False):
    session = db.query(DbSession).filter(DbSession.id == session_id).first()
    if not session:
        raise HTTPException(404, "Session not found")
    if user and session.owner != user and not allow_admin:
        raise HTTPException(404, "Session not found")
    return session
...
def setup_document_routes(session_manager, upload_handler=None) -> APIRouter:
    ...
    async def import_pdf(
        ...
        if session_id:
            ...
            try:
                _get_session_or_404(
                    db, session_id, user, allow_admin=is_admin_request(request, user)
                )
            finally:
                db.close()
        ...
        try:
            bgplx_text = strip_pdf_content_marker(_process_pdf(pdf_path, owner=user))
        except Exception:
            bgplx_text = None
    ...
    async def list_documents(request: Request, session_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if user and not is_admin_request(request, user):
                # Legacy documents may not have been backfilled yet; they are
                # visible only when their linked session belongs to this user,
                # never merely because the owner column is NULL.
                q = q.filter(
                    or_(
                        Document.owner == user,
                        and_(
                            Document.owner.is_(None),
                            DbSession.owner == user,
                        ),
                    )
                ).join(DbSession, Document.session_id == DbSession.id, isouter=True)
    ...
    async def extract_pdf_text(request: Request, doc_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            doc.current_content = head + bgplx_text.strip() + "\n"
            ...
            db.commit()
            return {"ok": True, "id": doc_id, "extracted": True, "chars": len(bgplx_text)}
        finally:
            db.close()
        ...
        try:
            ...
            for doc in docs:
                ...
                if title in ("new email", "new mail", "new message") or doc.language == "email":
                    bgplx_lines = [ln.strip() for ln in content.split("\n")
                    ...
                    has_real_body = any(not _is_filler(ln) for ln in bgplx_lines)
                    if bgplx_lines and not has_real_body:
                        _is_email_stub = True
    ...
    async def prepare_signed_reply(doc_id: str, request: Request):
        ...
        # as email_routes (GEPLEX_MAIL_ATTACHMENTS_DIR).
