...
from routes.email_helpers import (
    ...
    attachment_extract_dir, _email_cache_owner_clause, email_translation_bgplx_hash,
)
from routes.email_pollers import _start_poller

logger = logging.getLogger(__name__)

GEPLEX_MAIL_ORIGIN = "geplex-ui"
...
def _parse_list_unsubscribe_header(value: str | None) -> list[dict]:
    ...
    first-pass GepLex flow. HTTP unsubscribe links are useful evidence but
    ...
    try:
        ...
        try:
            row = conn.execute(
                ...
                FROM email_bgplx_preview_cache
...
def _email_preview_cache_put(owner: str, account_id: str | None, folder: str, uid: str, payload: dict):
    ...
    try:
        ...
        try:
            conn.execute(
                """
                INSERT INTO email_bgplx_preview_cache
...
def _apply_geplex_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-GepLex-Origin"] = GEPLEX_MAIL_ORIGIN
    if kind:
        msg["X-GepLex-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-GepLex-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _fixture_email_record(row: dict, uid_num: int, owner: str) -> dict:
        ...
        return {
            "uid": uid,
            "message_id": f"<fixture-email-{uid}-{owner_key}@fixtures.geplex.local>",
            "subject": subject,
    ...
    def _fixture_email_read(uid: str, folder: str, owner: str) -> dict | None:
        ...
        for e in _fixture_email_rows(owner):
            ...
            bgplx_html = "<html><body><p>" + html.escape(body).replace("\n", "<br>") + "</p></body></html>"
            return {
                "uid": str(uid),
                "folder": folder,
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the GepLex marker header, but include the subject
                # fallback too. The fallback uses a distinct GepLex prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-GepLex-Kind "reminder" SUBJECT "Reminder (GepLex):"{from_clause})',
                )
    ...
    def execute_unsubscribe(data: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            msg_out["Message-ID"] = email.utils.make_msgid(domain="geplex.local")
            _apply_geplex_headers(msg_out, "unsubscribe", uid)
        ...
        try:
            ...
            if cached_turns is None:
                try:
                    ...
                    cached_turns = parse_thread(bgplx_html, body)
    ...
    @router.delete("/geplex/reminders")
    async def delete_geplex_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as GepLex reminders."""
        if account_id:
            _assert_owns_account(account_id, owner)
        deleted = 0
        folders_checked = []
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-GepLex-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (GepLex):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (GepLex):")})'))
                            # Legacy reminders created before the GepLex
            ...
            logger.error(f"delete_geplex_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    def _load_geplex_attachment_source(db, kind: str, item_id: str, owner: str):
    ...
    @router.post("/compose-from-geplex")
    async def compose_from_geplex(data: dict, owner: str = Depends(require_owner)):
        """Stage an GepLex document or gallery image as a compose upload."""
        ...
        try:
            ...
            db = _SL()
            try:
                src = _load_geplex_attachment_source(db, kind, item_id, owner)
                if "path" in src:
            ...
            logger.error(f"Failed to stage GepLex attachment {kind}/{item_id}: {e}")
            return {"success": False, "error": "Mail operation failed"}

    @router.post("/compose-from-geplex-zip")
    async def compose_from_geplex_zip(data: dict, owner: str = Depends(require_owner)):
        """Stage several GepLex documents/gallery images as one zip attachment."""
        ...
        try:
            ...
            try:
                ...
                return _stage_compose_bytes("geplex-attachments.zip", content)
            finally:
                db.close()
        except HTTPException:
            raise
        except Exception as e:
            logger.error(f"Failed to stage GepLex zip attachment: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", geplex_kind=None, geplex_ref=None,
    ):
        ...
        if has_atts:
            ...
            bgplx_container = MIMEMultipart("alternative")
        else:
            outer = MIMEMultipart("alternative")
            bgplx_container = outer
        ...
        _apply_geplex_headers(outer, geplex_kind or "scheduled", geplex_ref)
        if in_reply_to:
            outer["In-Reply-To"] = in_reply_to
        if references:
            outer["References"] = references

        bgplx_container.attach(MIMEText(body or "", "plain", "utf-8"))
        bgplx_container.attach(MIMEText(_md_to_email_html(body or ""), "html", "utf-8"))

        if has_atts:
            outer.attach(bgplx_container)
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            # Bgplx-based account_id — dep can't see it, check here.
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, geplex_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("geplex_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="geplex.local")
        ...
        if req.geplex_kind:
            _apply_geplex_headers(outer, req.geplex_kind)
        ...
        bgplx_container.attach(MIMEText(req.body, "plain", "utf-8"))
        ...
        _html_part = (_sanitize_email_html(req.bgplx_html) if req.bgplx_html else None) \
            or _md_to_email_html(req.body)
        bgplx_container.attach(MIMEText(_html_part, "html", "utf-8"))
    ...
    async def save_draft(req: SendEmailRequest, owner: str = Depends(require_owner)):
        ...
        _draft_html = _sanitize_email_html(req.bgplx_html) if req.bgplx_html else None
        if _draft_html:
    ...
    async def translate_email(data: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            bgplx_hash = email_translation_bgplx_hash(body)
            try:
                _c = _sql3.connect(SCHEDULED_DB)
                owner_clause, owner_params = _email_cache_owner_clause(owner)
                row = _c.execute(
                    f"SELECT translation, same_language, model_used FROM email_translations "
                    f"WHERE bgplx_hash = ? AND target_language = ? AND {owner_clause}",
                    (bgplx_hash, target_language, *owner_params),
                ).fetchone()
                _c.close()
                if row:
                    if int(row[1] or 0):
            ...
            if "<<<SAME_LANGUAGE>>>" in content:
                try:
                    ...
                    _c.execute("""
                        ...
                        (bgplx_hash, owner, target_language, uid, folder, subject, sender,
                         translation, same_language, model_used, created_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
                        bgplx_hash, owner, target_language, data.get("uid", ""), data.get("folder", ""),
