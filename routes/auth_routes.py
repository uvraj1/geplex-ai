...
from src.settings import (
    ...
    DEFAULT_SETTINGS,
    get_settings_revision as _get_settings_revision,
...
_GMAIL_ADDRESS_RE = re.compile(
    r"^[a-z0-9](?:[a-z0-9.!#$%&'*+/=?^_`{|}~-]*[a-z0-9])?@gmail\.com$",
    re.IGNORECASE,
)
...
def _normalize_login_identifier(value: str) -> str:
    """Accept an existing username or email identifier at sign-in.

    Gmail-only validation belongs to account creation, not authentication.
    Login must remain compatible with existing accounts whose identifier is an
    email address from another domain.
    """
    identifier = (value or "").strip().lower()
    if not identifier:
        raise HTTPException(400, "Username or email address is required")
    if len(identifier) > 128 or any(char.isspace() for char in identifier):
        raise HTTPException(400, "Enter a valid username or email address")
    return identifier
...
def _cookie_samesite(request: Request) -> str:
    """Allow credentialed cookies from a separately hosted Cloudflare frontend."""
...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
    ...
    async def google_login(body: GoogleLoginRequest, request: Request, response: Response):
        """Verify a Firebase ID token and create the normal GepLex session."""
        try:
            import firebase_admin
            from firebase_admin import auth as firebase_auth
            if not firebase_admin._apps:
                service_account_json = os.getenv("GEPLEX_FIREBASE_SERVICE_ACCOUNT_JSON", "").strip()
            ...
            logger.exception("Invalid Firebase service-account configuration")
            raise HTTPException(503, "Google sign-in server credentials are invalid")
        except Exception:
            logger.exception("Google/Firebase token verification failed")
            raise HTTPException(401, "Google sign-in token is invalid or expired")
        email = _validate_gmail_address(decoded.get("email", ""))
        username = email
        if username not in auth_manager.users:
            created = await asyncio.to_thread(auth_manager.create_user, username, os.urandom(24).hex(), False)
            if not created:
                raise HTTPException(409, "Could not create Google account")
        token = await asyncio.to_thread(auth_manager.create_session_trusted, username)
        if not token:
            raise HTTPException(401, "Could not create login session")
        response.set_cookie(key=SESSION_COOKIE, value=token, httponly=True,
                            samesite=_cookie_samesite(request), secure=_secure_cookie(request),
                            path="/", max_age=TOKEN_TTL if body.remember else None)
        return {"ok": True, "username": username}
    ...
    async def admin_create_user(body: CreateUserRequest, request: Request):
        ...
        if gmail in RESERVED_USERNAMES:
            raise HTTPException(403, "Username is reserved")
        ok = auth_manager.create_user(gmail, body.password, body.is_admin)
        if not ok:
            raise HTTPException(409, "Username already taken")
        return {"ok": True}
    ...
    async def rename_user(username: str, body: RenameUserRequest, request: Request):
        ...
        new_username = _validate_gmail_address(body.username)
    ...
    async def get_settings_version():
        """Return the global configuration revision without exposing settings."""
        return {"version": _get_settings_revision()}
    ...
    async def test_integration_route(integration_id: str, request: Request):
        ...
        if preset == "ntfy":
            ...
            # base URL (e.g. `http://host:8091/geplex`) — otherwise
            # the topic gets appended after the path and we publish to
            # `/geplex/geplex` (which ntfy 404s on). ntfy itself
            ...
            headers = {
                "Title": "GepLex connectivity test",
            ...
            try:
                async with httpx.AsyncClient(timeout=8.0) as client:
                    r = await client.post(
                        full_url,
                        content="Connectivity test from GepLex. If you see this on your phone, ntfy is wired up correctly.",
                        headers=headers,
                    )
                if r.is_success:
