...
def _load_sam_backend():
    model_id = os.getenv("GEPLEX_SAM_MODEL", "facebook/sam-vit-base")
...
def _load_grounding_backend():
    model_id = os.getenv("GEPLEX_GROUNDING_MODEL", "google/owlvit-base-patch32")
...
def setup_gallery_routes() -> APIRouter:
    ...
    async def inpaint_proxy(request: Request):
        ...
        # Self-hosted diffusion server path. Newer GepLex image
            ...
            async with httpx.AsyncClient(timeout=240) as client:
                try:
                    ...
                    if r.status_code not in (404, 405):
                        ...
                        # GepLex diffusion_server.py has a dedicated
    ...
    async def harmonize_image(request: Request):
        ...
        bgplx_mask_b64 = body.get("bgplx_mask") or body.get("mask")
        ...
        harmonize_payload = {
            ...
            # `bgplx_mask` over `mask`, so sending both is safe.
            "strength": color_match,
        }
        if bgplx_mask_b64:
            harmonize_payload["bgplx_mask"] = bgplx_mask_b64
            harmonize_payload["mask"] = bgplx_mask_b64
        if seam_mask_b64:
