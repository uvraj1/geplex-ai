...
def _friendly_email_auth_error(protocol: str, host: str, error: object) -> str:
    ...
    if microsoft_basic_auth_failure:
        return (
            ...
            "Outlook/Office 365 IMAP/SMTP in most accounts. GepLex "
...
def _build_email_summary_messages(sender: str, subject: str, bgplx_for_llm: str) -> list[dict[str, str]]:
    return [
        {
            "role": "system",
            "content": (
...
async def _generate_email_summary(
    ...
    *,
    ...
    raw = await llm_call_async(
        url=url,
        model=model,
        messages=_build_email_summary_messages(sender, subject, bgplx_for_llm),
        temperature=0.3,
...
def require_owner(request: Request, account_id: str | None = Query(None)) -> str:
    ...
    request body or path must still call `_assert_owns_account(bgplx_id, owner)`
...
# Override at deploy time via GEPLEX_MAIL_ATTACHMENTS_DIR. Defaults to a
...
def email_translation_bgplx_hash(body: str) -> str:
...
def _init_scheduled_db():
    ...
    """)
    conn.execute("""
        CREATE TABLE IF NOT EXISTS email_bgplx_preview_cache (
            owner TEXT NOT NULL DEFAULT '',
            account_key TEXT NOT NULL DEFAULT '',
            folder TEXT NOT NULL,
            uid TEXT NOT NULL,
    ...
    """)
    conn.execute("""
        CREATE INDEX IF NOT EXISTS ix_email_bgplx_preview_message_id
        ON email_bgplx_preview_cache(owner, account_key, message_id)
    """)
    conn.execute("""
    ...
    try:
        ...
        if "geplex_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN geplex_kind TEXT")
...
_IMAP_TIMEOUT_SECONDS = _coerce_imap_timeout_seconds(os.environ.get("GEPLEX_IMAP_TIMEOUT_SECONDS"))
        ...
        for folder in ["INBOX", "Sent", "Archive", "Drafts"]:
            ...
            for raw_uid in uids:
                ...
                try:
                    ...
                    bgplx_text = (_extract_text(msg) or "").strip()
                    bgplx_text = re.sub(r"\n{3,}", "\n\n", bgplx_text)
                    if len(bgplx_text) > max_chars_per_email:
                        bgplx_text = bgplx_text[:max_chars_per_email].rstrip() + "…"
                ...
                if not bgplx_text and not atts_text:
                    continue

                lines = [f"— {folder} · {date_hdr} · Subject: {subj}"]
                if bgplx_text:
                    lines.append(bgplx_text)
                if atts_text:
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for GepLex-generated mail (e.g. reminder, scheduled).
    geplex_kind: Optional[str] = None
