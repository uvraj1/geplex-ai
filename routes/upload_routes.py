...
def setup_upload_routes(upload_handler):
    ...
    async def api_upload(
        ...
        if not isinstance(session_id, str):
            session_id = None
        if not isinstance(relative_paths, str):
            relative_paths = None
        if not files:
        ...
        try:
            ...
            raise HTTPException(400, "Invalid project file paths")
        if not isinstance(paths, list) or len(paths) not in (0, len(files)):
            raise HTTPException(400, "Project file paths do not match uploaded files")
        ...
        for file_index, u in enumerate(files):
            try:
                ...
                if paths:
                    relative_path = str(paths[file_index] or "").replace("\\", "/").lstrip("/")
                    if relative_path:
                        item["relative_path"] = relative_path[:1024]
                if gallery_id:
