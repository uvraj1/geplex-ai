...
GepLex keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/geplex-backup` tool
...
```bash
# Create a snapshot → backups/geplex-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/geplex-backup snapshot

# List existing snapshots (most recent first)
./scripts/geplex-backup list

# Check a tarball's integrity without extracting it
./scripts/geplex-backup verify backups/geplex-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/geplex-backup restore backups/geplex-backup-20260101-120000.tar.gz --yes
```
...
./scripts/geplex-backup snapshot --out /mnt/nas/geplex-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/geplex-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/geplex && ./scripts/geplex-backup snapshot --out "/mnt/nas/geplex-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `geplex-backup` therefore does not capture the Docker
