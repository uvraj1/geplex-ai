...
MIN_CONFIDENCE = 0.6
_MAX_LEARNED_SKILL_STEPS = 12
_SECRET_OR_SELF_MODIFYING_RE = re.compile(
    r"(?:api[_ -]?key|secret|password|private key|access token|credential|"
    r"modify (?:the )?(?:agent|assistant|system)|rewrite (?:your|its) own code|"
    r"disable (?:safety|approval|security))",
    re.IGNORECASE,
)
...
def _safe_learned_skill(data: dict) -> bool:
    """Keep autonomous learning procedural and non-privileged.

    Learned skills may improve repeatable computer workflows, but they must
    never become a channel for secrets, approval bypasses, or self-modifying
    instructions. Source changes remain explicit workspace actions with the
    normal approval and verification gates.
    """
    ...
    if not isinstance(steps, list) or not (3 <= len(steps) <= _MAX_LEARNED_SKILL_STEPS):
        return False
    if _SECRET_OR_SELF_MODIFYING_RE.search(text):
        return False
    return True
...
async def maybe_extract_skill(
    ...
    try:
        ...
        if not _safe_learned_skill(data):
            logger.info(
                "[skill-extract] '%s' rejected by autonomous-learning safety policy",
                title,
            )
            return None
