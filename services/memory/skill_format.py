...
Body sections (any subset; rendered as headings):
    ...
    in `bgplx_extra` and round-trips on save.
...
def parse_body(body: str) -> Dict[str, Any]:
    ...
    Returns:
        {
            ...
            "bgplx_extra":  str,         # anything not under a known heading
        }
    """
    ...
    for key, lines in sections:
        ...
        if key is None:
            extras = text.strip()
            if extras:
                out["bgplx_extra"] = (out["bgplx_extra"] + "\n\n" + extras).strip()
            continue
        if key == "when_to_use":
...
def emit_body(sections: Dict[str, Any]) -> str:
    ...
    extra = (sections.get("bgplx_extra") or "").strip()
    if extra:
        parts.append(extra)
    return "\n\n".join(parts) + ("\n" if parts else "")
...
class Skill:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        d["solution"] = (self.procedure[0] if self.procedure else "") if not self.bgplx_extra else self.bgplx_extra
    ...
    def from_markdown(cls, text: str, *, path: Optional[str] = None) -> "Skill":
        ...
        return cls(
            ...
            bgplx_extra=sections["bgplx_extra"],
            path=path,
        )
