...
def render_plain_pdf_markdown(upload_id: str, title: str, bgplx_text: Optional[str] = None) -> str:
    ...
    if bgplx_text and bgplx_text.strip():
        lines.append(bgplx_text.strip())
        lines.append("")
    return "\n".join(lines) + "\n"
...
def create_plain_pdf_document(
    ...
    content = render_plain_pdf_markdown(upload_id, title, bgplx_text)
    db = SessionLocal()
    try:
