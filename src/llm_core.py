...
def _local_model_gate_enabled() -> bool:
    return os.getenv("GEPLEX_LOCAL_MODEL_GATE", "true").lower() not in {"0", "false", "no", "off"}
...
def _configured_local_model_backends() -> List[str]:
    """Return configured local OpenAI-compatible model servers.

    When the bundled GepLex official endpoint is unreachable in the current
    environment, the app can still continue by retrying a local Ollama/LM Studio
    backend if one is configured. This keeps the project usable without silently
    failing on an unreachable public host.
    """
    candidates: List[str] = []
...
def _public_geplex_host(url: str) -> bool:
    return _host_match(url, "geplex.ai") or _host_match(url, "geplix.ai")


def _reachable_local_model_backend_for(url: str) -> Optional[str]:
    """Return a reachable local backend, if configured, when the official host is down."""
    if not _public_geplex_host(url):
        return None
    for candidate in _configured_local_model_backends():
        try:
            base = candidate.rstrip("/")
            probe = base if base.endswith("/v1") else f"{base}/v1"
            resp = httpx.get(f"{probe}/models", timeout=1.5)
            if resp.is_success:
                return candidate
        except Exception:
            continue
...
def _unreachable_upstream_message(url: str) -> str:
    """Return a user-facing message for a dead public GepLex endpoint."""
    host = _host_key(url)
    if _public_geplex_host(url):
        return (
            f"Official GepLex endpoint {host} is unreachable in this environment. "
            "Start a local Ollama/LM Studio server or set OLLAMA_BASE_URL / LM_STUDIO_URL to a reachable OpenAI-compatible model endpoint."
        )
    return f"Cannot reach {host}"


def _resolve_runtime_url(url: str) -> str:
    """Prefer a configured local backend when the public GepLex host is unavailable."""
    fallback = _reachable_local_model_backend_for(url)
    if fallback:
        return fallback
    return url
...
def _ollama_normalize_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt GepLex' canonical OpenAI-style messages to native Ollama /api/chat.

    Two shape mismatches silently break requests:

    1. Tool calls: GepLex carries `function.arguments` as a JSON *string*.
    ...
    2. Images (issue #4723): GepLex carries multimodal user content as an
...
def _detect_provider(url: str) -> str:
    ...
    if _host_match(url, "mistral.ai"):
        return "mistral"
    if _host_match(url, "generativelanguage.googleapis.com", "ai.google.dev", "googleapis.com"):
        return "gemini"
...
def _apply_local_cache_affinity(payload: Dict, url: str, session_id: Optional[str]) -> None:
    ...
    Sending a stable ``session_id`` (derived from the GepLex session) lets
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/geplex-dev/geplex")
        h.setdefault("X-OpenRouter-Title", "GepLex")
    if provider == "copilot":
    ...
    if provider == "gemini":
        ...
        if auth.startswith("Bearer "):
            ...
            if key:
                h.setdefault("x-goog-api-key", key)
    return h
...
# value returns HTTP 400. GepLex does not currently send the `thinking` mode
...
# GEPLEX_MISTRAL_REASONING_EFFORT (e.g. set to "medium" for cheaper chat).
_MISTRAL_REASONING_EFFORT = os.getenv("GEPLEX_MISTRAL_REASONING_EFFORT", "high")
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip GepLex-only metadata before sending messages to providers.
...
async def llm_call_async(
    ...
    if _is_host_dead(target_url):
        raise HTTPException(503, _unreachable_upstream_message(target_url) + " (cooldown active)")
    ...
    while attempt < max_retries:
        ...
        try:
            ...
            if _cooled or attempt >= max_retries:
                raise HTTPException(503, _unreachable_upstream_message(target_url) + f": {e}")
            await asyncio.sleep(LLMConfig.RETRY_DELAY)
...
async def _stream_llm_inner(url: str, model: str, messages: List[Dict], temperature: float = LLMConfig.DEFAULT_TEMPERATURE,
    ...
    if provider == "anthropic":
        ...
        # Effort level is configurable via GEPLEX_MISTRAL_REASONING_EFFORT
    ...
    if provider == "chatgpt-subscription":
        ...
        try:
            ...
            yield f'event: error\ndata: {json.dumps({"error": _unreachable_upstream_message(target_url), "status": 503})}\n\n'
