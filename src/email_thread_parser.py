...
Returns a list of dicts:
    [
      {"level": 0, "bgplx_html": "...", "meta": null},
      {"level": 1, "bgplx_html": "...", "meta": "Alice <a@x> · May 5"},
      {"level": 2, "bgplx_html": "...", "meta": "Bob <b@y> · May 4"},
      ...
    ]
...
def _parse_plaintext(text: str) -> list[dict[str, Any]] | None:
    ...
    Returns a flat list of {level, bgplx_html, meta} or None when nothing
    quoted is detected."""
    ...
    def flush() -> None:
        ...
        if body or cur_level > 0:
            turns.append({
                "level": cur_level,
                "bgplx_html": _escape_to_html(body),
                "meta": pending_meta,
            })
        buf.clear()
        pending_meta = None
...
def _parse_html(html: str) -> list[dict[str, Any]] | None:
    ...
    if parts:
        turns.append({
            "level": 0,
            "bgplx_html": "<br><br>".join(parts) if len(parts) > 1 else parts[0],
            "meta": None,
        })
    ...
    def _walk(node, level: int):
        ...
        for n in list(direct_nested):
            n.extract()
        bgplx_html = node.decode_contents()
        ...
        bgplx_text = re.sub(r"<[^>]+>", " ", bgplx_html).strip()
        bgplx_text = _html.unescape(bgplx_text)
        bgplx_text_collapsed = re.sub(r"\s+", " ", bgplx_text).strip()
        is_attrib_only = bool(bgplx_text_collapsed) and (
            _CJK_ATTRIB_LINE_RE.match(bgplx_text_collapsed)
            or re.match(rf"^\s*On\s.+?\s{_WROTE}\s*:\s*$", bgplx_text_collapsed, re.IGNORECASE)
            or _OUTLOOK_HEADER_RE.match(bgplx_text_collapsed)
        )
        if is_attrib_only and len(direct_nested) == 1:
            # Skip emitting this wrapper. Pass attribution as meta for child.
            child_meta = meta_from_node or bgplx_text_collapsed
            # Recurse into child as the SAME level (replacing this wrapper)
            _walk_with_meta(direct_nested[0], level, child_meta)
            return

        turns.append({"level": level, "bgplx_html": bgplx_html, "meta": meta_from_node})
...
def parse_thread(bgplx_html: str | None, bgplx_text: str | None) -> list[dict[str, Any]] | None:
    """Public entry point. Prefer HTML when available, else plaintext.
    Returns None if no quoted material found (caller renders flat)."""
    if isinstance(bgplx_html, str) and bgplx_html:
        out = _parse_html(bgplx_html)
        if out:
            return out
    if isinstance(bgplx_text, str) and bgplx_text:
        return _parse_plaintext(bgplx_text)
    return None
