...
INTERNAL_UPLOAD_URL_RE = re.compile(
    r"(?:geplex://attachment/|/api/upload/)"
...
class UploadHandler:
    def __init__(self, base_dir: str, upload_dir: str):
        ...
        # Per-IP per-minute cap. Zero means unlimited file count; burst abuse is
        # still separately bounded by max_concurrent_uploads and file-size/type
        # validation. Keeping this switch explicit lets deployments restore a
        # finite policy without changing the upload flow.
    ...
    def _renamed_upload_index_key(self, key: str, info: Dict[str, Any], old_owner: str, new_owner: str) -> str:
        ...
        # Fallback for rows without an explicit hash (should not happen in modern GepLex)
    ...
    def save_upload(self, u: UploadFile, client_ip: str, owner: str = None) -> dict:
        ...
        with self._upload_rate_lock:
            ...
            if self.upload_rate_limit > 0 and len(self.upload_rate_log[client_ip]) >= self.upload_rate_limit:
                raise HTTPException(
                    status_code=429,
