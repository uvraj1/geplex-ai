...
async def action_run_script(owner: str, script: str = "", host: str = "", **kwargs) -> Tuple[str, bool]:
    ...
    target_host = (host or os.getenv("GEPLEX_SCRIPT_HOST", "localhost")).strip()
...
async def action_email_auto_translate(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
        ...
        def _cached(bgplx_hash: str) -> bool:
            c = _sql3.connect(SCHEDULED_DB)
            try:
                owner_clause, owner_params = _email_cache_owner_clause(owner)
                row = c.execute(
                    f"SELECT 1 FROM email_translations "
                    f"WHERE bgplx_hash = ? AND target_language = ? AND {owner_clause} LIMIT 1",
    ...
    try:
        ...
        try:
            ...
            for n in notes:
                ...
                bgplx_parts = []
                if n.content:
                    bgplx_parts.append(n.content[:400])
                ...
                if n.items:
                    try:
                        ...
                        if pending:
                            bgplx_parts.append("Pending:\n" + "\n".join(f"- {t}" for t in pending[:8]))
                    except Exception:
                        pass
                body = "\n\n".join(p for p in bgplx_parts if p) or title
                try:
                    ...
                    for uid_b in uids:
                        ...
                        try:
                            ...
                            if not raw:
                                ...
                                continue
                            msg = _email_mod.message_from_bytes(raw)
                            # Skip GepLex-generated reminders so the scanner
                            ...
                            _gplx_origin = (msg.get("X-GepLex-Origin") or "").strip().lower()
                            _gplx_kind = (msg.get("X-GepLex-Kind") or "").strip().lower()
                            ...
                            if (_gplx_origin == "geplex-ui" or _gplx_kind == "reminder"
                                    or _raw_subj.startswith("reminder (geplex):")
                                ...
                                if msg.is_multipart():
                                    for part in msg.walk():
                                        if part.get_content_type() == "text/plain":
                                            bgplx_snippet = part.get_payload(decode=True).decode("utf-8", errors="ignore")[:1600]
                                            break
                                else:
                                    bgplx_snippet = (msg.get_payload(decode=True) or b"").decode("utf-8", errors="ignore")[:1600]
                            except Exception:
                                bgplx_snippet = ""
                            results[-1].update({
                                "subject": subject,
...
BUILTIN_ACTION_INFO = {
    ...
    "run_script": "Run a script locally or on GEPLEX_SCRIPT_HOST",
