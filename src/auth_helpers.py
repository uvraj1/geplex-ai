...
def effective_user(request: Request) -> Optional[str]:
    ...
    Cookie sessions resolve to the logged-in username. Bearer ``gplx_`` callers
...
def is_admin_request(request: Request, user: Optional[str] = None) -> bool:
    """Return whether the authenticated request belongs to an administrator."""
    username = user if user is not None else get_current_user(request)
    if not username or request is None:
        return False
    app = getattr(request, "app", None)
    auth_mgr = getattr(getattr(app, "state", None), "auth_manager", None)
    checker = getattr(auth_mgr, "is_admin", None)
    return bool(callable(checker) and checker(username))
