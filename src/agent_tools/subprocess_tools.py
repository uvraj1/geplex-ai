    ...
    if IS_WINDOWS:
        ...
        if not bash:
            raise RuntimeError(
                ...
                "install Git for Windows and restart GepLex"
            )
...
def _tmux_session_name(session_id: Optional[str]) -> str:
    ...
    return f"gplx-agent-{raw[:80] or 'default'}"
...
async def _run_tmux_bash(
    ...
    *,
    ...
    start_marker = f"__GEPLEX_CMD_START_{stamp}__"
    end_prefix = f"__GEPLEX_CMD_END_{stamp}__:"
    wrapped = (
        f"printf '\\n{start_marker}\\n'\n"
        f"{content}\n"
        f"__gplx_rc=$?\n"
...
def _clean_tmux_command_output(text: str, wrapped_command: str) -> str:
    ...
    for line in lines:
        ...
        if stripped in wrapped_lines:
            continue
        if stripped.startswith("__gplx_rc=") or stripped.startswith("printf "):
            continue
