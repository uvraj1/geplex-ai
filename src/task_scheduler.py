...
class TaskScheduler:
        ...
        try:
            ...
            if gate_foreground:
                ...
                if waiting and waiting.status == "queued":
                    waiting.result = "Queued — waiting for GepLex to be idle…"
                    db.commit()
                ...
                async def _cancel_if_foreground_active():
                    ...
                    while True:
                        ...
                        if has_foreground_activity():
                            ...
                            logger.info("Task '%s' interrupted because GepLex became active", task.name)
                            if current_task:
                                current_task.cancel()
                            return
            ...
            try:
                ...
                msg = (
                    "Paused because GepLex became active"
    ...
    async def _deliver_via_email(self, output: str, task, result: str):
        ...
        try:
            ...
            msg["X-GepLex-Origin"] = "geplex-ui"
            msg["X-GepLex-Kind"] = "task"
            msg["X-GepLex-Ref"] = str(task.id)
    ...
    async def _deliver_via_mcp(self, tool_name: str, task, result: str):
        ...
        args = {
            ...
            "body": result,
            "headers": {
                "X-GepLex-Origin": "geplex-ui",
                "X-GepLex-Kind": "task",
                "X-GepLex-Ref": str(task.id),
            },
        }
        if recipient:
        ...
        try:
            ...
            if exit_code != 0:
                ...
                logger.info(
                    ...
                    f"(recipient_set={bool(recipient)}, body={bgplx_len}b, reply={stdout[:200]!r})"
                )
        except Exception as e:
    ...
    async def stop_background_tasks_for_foreground(self, *, reason: str = "GepLex became active") -> int:
        ...
        user opens or uses GepLex, foreground interaction wins immediately.
