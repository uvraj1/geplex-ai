            ...
            if mime == "application/pdf":
                ...
                if session_id:
                    try:
                        ...
                        try:
                            pdf_bgplx_text = strip_pdf_content_marker(_process_pdf(path, owner=owner))
                        except Exception:
                            pdf_bgplx_text = None
                        ...
                        bgplx_for_chat = (pdf_bgplx_text or "").strip()
                        truncated_marker = ""
                        if bgplx_for_chat and len(bgplx_for_chat) > _MAX_INLINE_CHARS:
                            bgplx_for_chat = bgplx_for_chat[:_MAX_INLINE_CHARS]
                            truncated_marker = (
                        ...
                        if is_form:
                            ...
                            if doc_id:
                                ...
                                if bgplx_for_chat:
                                    extracted_text += (
                                        f"\n\n[PDF content — {title}]:\n{bgplx_for_chat}{truncated_marker}"
                                    )
                        else:
