// ============================================
// GepLex UI — Main Application Orchestrator
...
const API_BASE = window.__GEPLEX_API_BASE || window.location.origin;
let _globalSettingsSyncStarted = false;
let _globalSettingsRevision = null;

function startGlobalSettingsSync() {
  if (_globalSettingsSyncStarted) return;
  _globalSettingsSyncStarted = true;
  const check = async () => {
    try {
      const response = await fetch(`${API_BASE}/api/auth/settings-version`, {
        credentials: 'same-origin',
        cache: 'no-store',
      });
      if (!response.ok) return;
      const data = await response.json();
      const revision = String(data.version || '');
      if (_globalSettingsRevision === null) {
        _globalSettingsRevision = revision;
      } else if (revision && revision !== _globalSettingsRevision) {
        // Reloading applies the admin's global configuration while browser
        // storage keeps each user's personal UI/theme preferences intact.
        window.location.reload();
      }
    } catch (_) {
      // A temporary poll failure must not interrupt an active chat session.
    }
  };
  check();
...
function _isForegroundChatBusy() {
  ...
  return !!window.__geplexChatBusy
    || Date.now() < (window.__geplexChatBusyUntil || 0)
...
function _submitMobileQueuedInput(input) {
  ...
  window.__geplexQueueStreamingSubmit = now;
...
try {
  const cachedDefaultChat = JSON.parse(localStorage.getItem('geplex-default-chat-cache') || 'null');
  if (cachedDefaultChat && cachedDefaultChat.endpoint_url && cachedDefaultChat.model) {
    _defaultChat = cachedDefaultChat;
    window.__geplexDefaultChat = cachedDefaultChat;
  }
} catch (_) {}
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try {
        window.__geplexDefaultChat = d;
        localStorage.setItem('geplex-default-chat-cache', JSON.stringify(d));
      } catch (_) {}
      return d;
    }
  } catch (_) {}
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  const _overflowProject = el('overflow-project-btn');
  if (_overflowProject) _overflowProject.addEventListener('click', () => {
    fileHandlerModule.openProjectPicker();
    if (typeof window.__geplexSetChatMode === 'function') window.__geplexSetChatMode('agent');
    if (uiModule.showToast) uiModule.showToast('Code Agent ready for your project');
  });
  el('file-input').addEventListener('change', async (e)=>{
    await fileHandlerModule.addFiles(Array.from(e.target.files || []));
    e.target.value = '';
  });
  el('project-folder-input')?.addEventListener('change', async (e)=>{
    const files = Array.from(e.target.files || []);
    if (files.length) {
      await fileHandlerModule.addFiles(files, { skipCrop: true });
      if (uiModule.showToast) uiModule.showToast(`${files.length} project files ready to upload`);
    }
    e.target.value = '';
  ...
  // their `.open`; ephemeral bgplx-appended menus are removed. Full modals/
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'GepLex Chat';
  ...
  fetch(`${API_BASE}/api/auth/status`, { credentials: 'same-origin' })
    ...
    .then(d => {
      ...
      document.body.classList.toggle('is-admin', window._isAdmin);
      document.body.classList.toggle('is-authenticated', !!d.username || window._isAdmin);
      const appearanceTab = document.querySelector(
        '#settings-modal .settings-nav-item[data-settings-tab="appearance"]'
      );
      if (appearanceTab) appearanceTab.hidden = !window._isAdmin;
      if (!window._isAdmin) {
        const appearancePanel = document.querySelector(
          '#settings-modal [data-settings-panel="appearance"]'
        );
        if (appearancePanel) appearancePanel.classList.add('hidden');
      }
      ...
      if (d.username || d.is_admin) {
        startGlobalSettingsSync();
      }
    })
    .catch(() => {});
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('geplex-model-sort', mode);
  ...
  const _prefetchedFeatures = sessionStorage.getItem('gplx-prefetch-features');
  sessionStorage.removeItem('gplx-prefetch-features');
  ...
  window._initSettingsReady = getSettings()
    .then(settings => {
      ...
      const browserVoiceAvailable = 'speechSynthesis' in window;
      const ttsOff = !browserVoiceAvailable && (settings.tts_enabled === false || !settings.tts_provider || settings.tts_provider === 'disabled');
  ...
  if (toolMemoryBtn && memoryModal) {
    toolMemoryBtn.addEventListener('click', () => {
      if (!window._isAdmin) {
        uiModule.showToast('Brain is available to administrators only');
        return;
      }
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  function applyModeToToggles(mode) {
    ...
    window.__geplexSetChatMode = setMode;
  ...
  (function initPlanToggle() {
    ...
    window.__geplexSetPlanMode = (active) => setPlanMode(active, { silent: true });
    const statusToggle = el('plan-mode-status-toggle');
    if (btn) {
      btn.addEventListener('click', () => {
        const st = loadToggleState();
    ...
    if (msgInput && !msgInput._geplexPlanTabToggle) {
      msgInput._geplexPlanTabToggle = true;
    ...
    if (chatBar && !chatBar._geplexPlanSwipeToggle) {
      chatBar._geplexPlanSwipeToggle = true;
      let touchStartX = 0;
      let touchStartY = 0;
      let touchStartAt = 0;
  ...
  const SPLASH_COUNT_KEY = 'geplex-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    const TOOLBAR_HIDE_WIDTH = 160;
	    ...
	    function setComposerPlaceholder(width) {
	      ...
	      textarea.setAttribute('placeholder', width < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message GepLex...');
	    }

	    if (_isMobile && textarea && !textarea._geplexPlanPlaceholderHint) {
	      textarea._geplexPlanPlaceholderHint = true;
	      setInterval(() => {
  ...
  // TTS mode is intentionally not exposed in the overflow menu.
  (function initTTSToggle() {
    ...
    ttsBtn.addEventListener('click', () => {
      ...
      if (isActive && window.aiTTSManager &&
          !window.aiTTSManager.available && 'speechSynthesis' in window) {
        window.aiTTSManager.useBrowserTTS = true;
        window.aiTTSManager.available = true;
      }
      if (!isActive && voiceRecorderModule.getIsRecording()) {
        voiceRecorderModule.stopRecording();
      }
      ttsBtn.classList.toggle('active', isActive);
      document.body.classList.toggle('voice-chat-active', isActive);
      ttsBtn.setAttribute('aria-pressed', String(isActive));
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (!beforeNobody.nobgplx_prev_mode) beforeNobody.nobgplx_prev_mode = beforeNobody.mode || 'agent';
        ...
        if (typeof window.__geplexSetChatMode === 'function') {
          window.__geplexSetChatMode('chat');
        } else {
        ...
        const _restoreMode = _ts.nobgplx_prev_mode || 'agent';
        delete _ts.nobgplx_prev_mode;
        ['web_agent', 'bash_agent', 'web_chat', 'bash_chat'].forEach(k => {
          if (_ts[k] === false) delete _ts[k];
        });
        Storage.setJSON(Storage.KEYS.TOGGLES, _ts);
        if (typeof window.__geplexSetChatMode === 'function') {
          window.__geplexSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'geplex-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'geplex-toolbar-visibility';
    try {
  ...
  if (textarea) {
    ...
    window.addEventListener('geplex:chat-busy-change', () => _syncMobileEnterKeyHint(textarea));
      ...
      if (_shouldQueueFromMobileEnter(e, textarea) || (e.key === 'Enter' && !e.shiftKey && !e.isComposing && !isMobile)) {
        ...
        if (form) {
          if (_isForegroundChatBusy() && textarea.value && textarea.value.trim()) {
            ...
            window.__geplexQueueStreamingSubmit = Date.now();
          }
          _submitChatFormDirect(form);
        }
      }
...
// ============================================
function startGepLexApp() {
  tasksModule?.startNotificationPolling?.();
  if (window.__geplexAppStarted) return;
  window.__geplexAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__geplexChatBusyUntil = Math.max(window.__geplexChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('geplex-doc-open-' + _curSession) === '1') {
  ...
  document.querySelectorAll('.welcome-action[data-prompt]').forEach((button) => {
    button.addEventListener('click', () => {
      if (!messageInput) return;
      const action = button.dataset.action || '';
      const prompt = button.dataset.prompt || '';
      if (typeof window.__geplexSetChatMode === 'function' && action !== 'latest') {
        window.__geplexSetChatMode('agent');
      }
      if (typeof window.__geplexSetPlanMode === 'function') {
        window.__geplexSetPlanMode(action === 'plan');
      }
      if (action === 'latest') {
        if (typeof window.__geplexSetChatMode === 'function') window.__geplexSetChatMode('agent');
  ...
  window._geplexBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  function _updateSendBtnIcon() {
    ...
    let newMode;
    if (!hasText && !hasFiles && (_isSttEnabled() || document.body.classList.contains('voice-chat-active'))) {
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', (e) => {
      ...
      if (sendBtn.dataset.mode === 'streaming') {
        if (hasText) window.__geplexQueueStreamingSubmit = Date.now();
        handleSubmit(e);
        return;
      }
      ...
      if (!hasText && !hasFiles && _isSttEnabled()) {
        ...
        voiceRecorderModule.startRecording(
          ...
          uiModule.showError,
          (transcript) => {
            // Voice chat submits the browser/server transcript through the same
            // authenticated chat path as typed messages.
            if (document.body.classList.contains('voice-chat-active')) {
  ...
  const runNonCriticalStartup = (fn, delay = 4000) => {
    let tries = 0;
    const run = () => {
      const busy = !!window.__geplexChatBusy
        || Date.now() < (window.__geplexChatBusyUntil || 0)
        || !!document.querySelector('.send-btn[data-mode="streaming"], .send-btn.send-pending');
      if (busy && tries < 12) {
        tries += 1;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startGepLexApp, { once: true });
} else {
  startGepLexApp();
}
