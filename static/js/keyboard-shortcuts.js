...
export function initKeyboardShortcuts(modules) {
  ...
  window._geplexKeybinds = { ..._defaultKeybinds };

  // Load saved keybinds
  getSettings()
    .then(s => { if (s.keybinds) window._geplexKeybinds = { ..._defaultKeybinds, ...s.keybinds }; })
    .catch(() => {});
  ...
  document.addEventListener('keydown', (e) => {
    const kb = window._geplexKeybinds;
      ...
      uiModule.styledConfirm('Delete this session?', { confirmText: 'Delete', danger: true }).then(ok => {
        ...
        fetch(`${API_BASE}/api/session/${sid}`, { method: 'DELETE' }).then(async () => {
          ...
          if (nextSession) {
            ...
            el('current-meta').textContent = 'GepLex Chat';
