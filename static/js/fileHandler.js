...
// Zero means unlimited files. Individual file size/type checks and the
// server-side abuse protections still apply.
const MAX_FILES = 0;
const MAX_VISIBLE = 3;
let _expanded = false;
...
export async function uploadPending(opts = {}) {
  ...
  const relativePaths = pendingFiles.map(f => f.webkitRelativePath || '');
  if (relativePaths.some(Boolean)) fd.append('relative_paths', JSON.stringify(relativePaths));
...
export async function addFiles(files, opts = {}) {
  for (const f of files) {
    if (MAX_FILES > 0 && pendingFiles.length >= MAX_FILES) {
