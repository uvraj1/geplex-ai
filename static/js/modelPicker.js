...
const RECENT_KEY = 'geplex-model-recent';
const FAVORITES_KEY = 'geplex-model-favorites';
const RECENT_MAX = 5;
...
function _initModelPickerDropdown() {
  ...
  async function _refreshLocalProbe() {
    try {
      if (window.__geplexChatBusy || Date.now() < (window.__geplexChatBusyUntil || 0)) return;
    } catch (_) {}
    const now = Date.now();
  ...
  const _collapsedProviders = new Set(_loadList('geplex-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('geplex-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
...
async function _pick(m) {
    _defaultPendingSeq++;
    try {
      window.__geplexLastPickedRoute = {
        model: m.mid || '',
        endpoint_url: m.url || '',
    ...
    try { window.__geplexModelSwitchPromise = switchPromise; } catch (_) {}
    const finishSwitch = () => {
      try {
        if (switchDone) switchDone();
        if (window.__geplexModelSwitchPromise === switchPromise) delete window.__geplexModelSwitchPromise;
      } catch (_) {}
    };
    ...
    try { document.dispatchEvent(new CustomEvent('geplex:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('geplex:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  if (!modelId && !currentSessionId && !_pendingChat && _deps.setPendingChat) {
    ...
    try {
      cachedDefault = window.__geplexDefaultChat || null;
    } catch (_) {}
    if (!cachedDefault || !cachedDefault.endpoint_url || !cachedDefault.model) {
      try {
        cachedDefault = JSON.parse(localStorage.getItem('geplex-default-chat-cache') || 'null');
      } catch (_) {}
    }
    if (cachedDefault && cachedDefault.endpoint_url && cachedDefault.model) {
  ...
  // SECURITY: deliberately NOT auto-injecting `geplex-model-favorites[0]`
