...
const _COST_LEDGER_LOCK = 'geplex-session-cost-ledger';
...
const _ACTION_RECENTS_KEY = 'geplex-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'geplex-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'GepLex';
      ...
      row.addEventListener('click', () => {
        if (isToolApproval) {
          ...
          if (onSubmit) {
            ...
            card.remove();
            document.dispatchEvent(new CustomEvent('geplex:tool-approval', { detail }));
          }
        } else {
          send(label);
        }
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'GepLex' : modelRouteLabel(
