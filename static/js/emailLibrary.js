    ...
    <div class="email-settings-section email-settings-style-section">
      <div class="email-settings-section-head">
        <div>
          ...
          <div class="email-settings-section-desc">Used when GepLex drafts replies for you.</div>
        </div>
      </div>
      <label class="email-settings-field">
...
document.addEventListener('geplex:email-filter-tag', (e) => {
...
try { window.__geplexGetActiveEmailContext = _getActiveEmailContext; } catch (_) {}
...
window.addEventListener('geplex-reminder-channel-changed', (e) => {
...
const _LIB_SESSION_CACHE_PREFIX = 'geplex.email.list.';
const _LIB_SESSION_CACHE_TTL_MS = 10 * 60 * 1000;
const _LIB_LAST_ACCOUNT_KEY = 'geplex.email.lastAccountId';
...
function _isChatInteractionBusy() {
  try {
    if (window.__geplexChatBusy) return true;
    const until = Number(window.__geplexChatBusyUntil || 0);
...
function _scheduleEmailPrewarm(task, { delay = 0 } = {}) {
  ...
  window.addEventListener('geplex:chat-busy-change', handlePriorityChange);
  document.addEventListener('visibilitychange', handlePriorityChange);
  _libPrewarmDetachPriorityListeners = () => {
    window.removeEventListener('geplex:chat-busy-change', handlePriorityChange);
...
function _publishActiveAccount() {
  try { window.__geplexActiveEmailAccount = state._libAccountId || null; } catch (_) {}
  try {
...
export function openEmailLibrary(opts = {}) {
        ...
        <div class="admin-card" style="flex:1;flex-direction:column;display:flex;overflow:hidden;">
          ...
          <div class="memory-toolbar">
            <div class="memory-category-filters">
              ...
              <button class="memory-toolbar-btn email-reminders-clear-btn hidden" id="email-reminders-clear-btn" title="Permanently delete GepLex reminder emails">
            ...
            <div class="email-search-wrap" style="position:relative;flex:1;min-width:140px;">
              ...
              <button class="memory-toolbar-btn email-reminder-toggle-inline hidden" id="email-reminder-btn" title="Show GepLex reminder emails">
  ...
  if (tagsToggle) {
    ...
    tagsToggle.addEventListener('click', () => {
      ...
      localStorage.setItem('geplex.email.showTags', state._libShowTags ? '1' : '0');
      tagsToggle.classList.toggle('active', !!state._libShowTags);
      tagsToggle.setAttribute('aria-pressed', String(!!state._libShowTags));
      _renderGrid();
      document.dispatchEvent(new CustomEvent('geplex:email-tags-toggle', { detail: { show: state._libShowTags } }));
    });
  }
  document.getElementById('email-reminders-clear-btn')?.addEventListener('click', async () => {
    const ok = await styledConfirm('Permanently delete all GepLex reminder emails?', {
      confirmText: 'Delete',
      cancelText: 'Cancel',
      danger: true,
    });
    if (!ok) return;
    try {
      const res = await fetch(`${API_BASE}/api/email/geplex/reminders?permanent=1${_acct()}`, {
        method: 'DELETE',
        credentials: 'same-origin',
      });
      const data = await res.json().catch(() => ({}));
...
async function _toggleCardPreview(card, em) {
  ...
  try {
    ...
    reader.innerHTML = `
      ...
      ${attsHtml}
      <div class="email-reader-body${data.bgplx_html ? ' html-body' : ''}">${_safeRenderEmailBody(data)}</div>
    `;
    _markEmailReaderActive(reader);
    reader.classList.remove('email-card-reader-loading');
    reader.style.minHeight = '';
...
const _BUBBLES_DISABLED_KEY = 'geplex.email.bubblesDisabled';
...
function _renderEmailBody(data) {
  ...
  const isHtml = !!data.bgplx_html;
  let rendered;
  if (isHtml) {
    rendered = _sanitizeHtml(data.bgplx_html);
  } else {
...
function _safeRenderEmailBody(data) {
  try {
    ...
    if (data?.bgplx_html) return _prepareEmailInlineImages(_sanitizeHtml(data.bgplx_html));
...
function _renderTurnsAsBubbles(turns, data) {
  ...
  const rows = ordered.map((t, i) => {
    ...
    return (
      ...
      +   head
      +   `<div class="email-bubble-body">${_sanitizeHtml(t.bgplx_html || '')}</div>`
      + `</div>`
      + (isMine ? avatar : '')
      + `</div>`
    );
...
/**
 * Render server-cached thread turns (list of {level, bgplx_html, meta})
...
function _renderTurnsFromServer(turns) {
  ...
  for (const t of turns) {
    if (t.level === 0) {
      ...
      out += _sanitizeHtml(t.bgplx_html || '');
    } else {
      ...
      if (!stack.length || stack[stack.length - 1].level < t.level) {
        stack.push({ level: t.level, meta: t.meta, html: _sanitizeHtml(t.bgplx_html || '') });
      } else {
        stack[stack.length - 1].html += _sanitizeHtml(t.bgplx_html || '');
...
const _SUMMARY_COLLAPSED_KEY = 'geplex.email.summaryCollapsed';
