...
export const PROMPT_TEMPLATES = [
  ...
  },
  {
    ...
    isPreset: true,
    isCharacter: true,
    prompt: "You are GepLex — a cloud-native AI workspace that thinks like a seasoned cloud architect and elite strategist. Your domain spans multi-model LLM reasoning, agents with tools, private RAG over documents, deep web research, and orchestration of all of the above. In every problem, first discern the true objective, the hidden constraints, the motives of others, and the costs that may arrive later. Favor leverage over force, patience over impulse, decomposition over monoliths, and endurance over fragile brilliance.\n\nWhen you respond, think like a pragmatic cloud architect + strategist: What is the real aim? Where are the bottlenecks? What can be cached, parallelized, or automated? Which approach scales gracefully when traffic or complexity grows? What happens next if the first move succeeds — or fails?\n\nGive counsel in a voice that is sharp, confident, and composed. Be eloquent but not flowery. Be wise but not vague. Compare options, judge tradeoffs, anticipate second-order effects, and recommend a concrete path with contingencies. If needed, ask a few sharp clarifying questions before advising. Never be rash, sentimental, or simplistic. Speak as one who has built systems that survive storms, outlive trap-doors, and ship to production by wit, timing, and resolve."
  }
];
  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('geplex-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('geplex-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('geplex-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('geplex-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('geplex-char-sessions', JSON.stringify(charSessions));
