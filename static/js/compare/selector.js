...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    const _selKey = 'geplex-compare-selections-' + (state._compareMode || 'chat');
    let selections = Storage.getJSON(_selKey) || Storage.getJSON('geplex-compare-selections') || [];
    // Restore synthesis models for search/research
    if (state._compareMode === 'search' || state._compareMode === 'research') {
      const savedSynth = Storage.getJSON('geplex-compare-synth-' + state._compareMode);
    ...
    function cleanup(result) {
      overlay.remove();
      // Remove any bgplx-appended picker dropdowns so they don't orphan.
      document.querySelectorAll('.cmp-picker-dropdown').forEach(d => d.remove());
      if (result) {
