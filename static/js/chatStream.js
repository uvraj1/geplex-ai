...
document.addEventListener('geplex:tool-approval', () => {
...
export function handleUIControl(uiData) {
  ...
  try {
    if (uiEvent === 'toggle' || uiData.ui_event === 'toggle') {
      ...
      document.querySelectorAll('.geplex-highlight').forEach(function(e) { e.classList.remove('geplex-highlight'); });
      document.querySelectorAll('.geplex-hl-label').forEach(function(e) { e.remove(); });
      var target = document.querySelector(uiData.selector);
      if (target) {
        target.classList.add('geplex-highlight');
        target.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        if (uiData.label) {
          var lbl = document.createElement('div');
          lbl.className = 'geplex-hl-label';
