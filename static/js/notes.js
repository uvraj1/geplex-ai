...
let _searchQuery = '';
let _viewMode = (typeof localStorage !== 'undefined' && localStorage.getItem('geplex-notes-view')) || 'list'; // 'list' or 'grid'
...
const REMINDER_FIRED_KEY = 'geplex-notes-reminder-fired';
...
const REMINDER_GLOWED_KEY = 'geplex-notes-reminder-glowed';
...
const REMINDER_PENDING_HIGHLIGHT_KEY = 'geplex-notes-reminder-pending-highlight';
const REMINDER_ACTIVE_HIGHLIGHT_KEY = 'geplex-notes-reminder-active-highlight';
...
const REMINDER_DISMISSED_AT_KEY = 'geplex-notes-reminder-dismissed-at';
const NOTES_FIRST_OPEN_HINT_KEY = 'geplex-notes-first-open-hint-v1';
...
function _bringNotesToFront(pane = document.getElementById('notes-pane')) {
  ...
  try {
    window.dispatchEvent(new CustomEvent('geplex:modal-opened', {
...
function _fireReminder(note) {
  ...
  const showLocal = (body) => {
    if ('Notification' in window && Notification.permission === 'granted') {
      try {
        const n = new Notification(title, { body, tag: 'note-' + note.id, icon: '/favicon.ico' });
...
export function openPanel() {
  ...
  if (viewBtn) {
    ...
    viewBtn.addEventListener('click', () => {
      ...
      try { localStorage.setItem('geplex-notes-view', _viewMode); } catch {}
      pane.classList.toggle('notes-view-grid', _viewMode === 'grid');
      _setViewLabel();
      requestAnimationFrame(() => _applyMasonry(document.querySelector('#notes-pane .notes-pane-body')));
    });
...
const _DRAFT_PREFIX = 'geplex-note-draft-';
