...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('geplex:auto-select-model', {
      detail: {
...
async function loadEndpoints() {
  ...
  try {
    ...
    const rowHtml = data.map(ep => {
      ...
      const protectedOfficialEndpoint = String(ep.id) === 'geplex-ai-core';
        ...
        <div class="admin-user-row${ep.is_enabled ? '' : ' admin-ep-disabled'}${justAddedClass}" data-adm-ep-id="${ep.id}">
          <div style="display:flex;align-items:center;justify-content:space-between;${hasModels ? 'cursor:pointer;' : ''}padding:4px 0;" data-adm-ep-header="${ep.id}">
            ...
            <div style="display:flex;gap:4px;align-items:center;">
              ...
              ${protectedOfficialEndpoint ? '' : `<button class="admin-btn-delete" data-adm-del-ep="${ep.id}" data-adm-ep-online="${ep.online ? '1' : '0'}">Delete</button>`}
    ...
    queryAll('[data-adm-del-ep]').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const epId = btn.dataset.admDelEp;
        const isOffline = btn.dataset.admEpOnline === '0';
        if (String(epId) === 'geplex-ai-core') {
          if (uiModule && uiModule.showToast) uiModule.showToast('The built-in GepLex system model cannot be deleted.', 4000);
          return;
        }
        if (!isOffline) {
          let deps = [];
          try {
            const depRes = await fetch('/api/model-endpoints/' + epId + '/dependents', { credentials: 'same-origin' });
            const depData = await depRes.json();
            deps = depData.dependents || [];
          } catch (_) { /* proceed without warning */ }
          let msg = 'Delete this endpoint?';
          if (deps.length) {
...
function initEndpointForm() {
  ...
  document.querySelectorAll('.adm-quickstart-section').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'geplex.addModels.' + sec.id + '.open';
    let open = false;
  ...
  try {
    ...
    list.querySelectorAll('[data-adm-del-token]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        try { window.dispatchEvent(new CustomEvent('geplex-integrations-changed')); } catch (_) {}
      });
    });
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'geplex_backup.json';
...
async function loadLearningAI() {
  const res = await fetch('/api/admin/learning-ai', { credentials: 'same-origin' });
  if (!res.ok) return;
  const data = await res.json();
  const status = document.getElementById('learning-status');
  if (status && data.configured) {
    status.textContent = `Default teacher model: ${data.teacher_model}`;
  }
}

function initLearningAI() {
  const status = document.getElementById('learning-status');
  const json = (id) => document.getElementById(id);
  const readResponse = async (res) => {
    let data = {};
    try { data = await res.json(); } catch (_) {}
    if (!res.ok) {
      const detail = data.detail || data.error || `Request failed (HTTP ${res.status})`;
      throw new Error(typeof detail === 'string' ? detail : JSON.stringify(detail));
    }
    return data;
  };
  const setBusy = (busy) => {
    ['learning-test', 'learning-teach'].forEach((id) => {
      const button = json(id);
      if (button) button.disabled = busy;
    });
  };
  const test = async () => {
    setBusy(true);
    status.textContent = 'Testing default model as teacher...';
    const res = await fetch('/api/admin/learning-ai/test', {method: 'POST', credentials: 'same-origin'});
    const data = await readResponse(res);
    status.textContent = `Teacher connected: ${data.response}`;
    setBusy(false);
  };
  const teach = async () => {
    setBusy(true);
    status.textContent = 'Analyzing transcript with default model...';
    const res = await fetch('/api/admin/learning-ai/teach', {
      method: 'POST', credentials: 'same-origin', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({transcript: json('learning-transcript').value})
    });
    const data = await readResponse(res);
    status.textContent = `Stored ${data.facts_stored} memories and ${data.skills_stored} skills.`;
    setBusy(false);
  };
  [['learning-test', test], ['learning-teach', teach]].forEach(([id, fn]) => {
    const button = json(id);
    if (button) button.addEventListener('click', () => fn().catch(e => {
      setBusy(false);
...
function initAll() {
  ...
  const inits = [
    ...
    () => settingsModule.initIntegrations(), initLearningAI
  ];
