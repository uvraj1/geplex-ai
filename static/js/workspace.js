...
function _render(data) {
  ...
  const project = _modal.querySelector('#workspace-project');
  if (project) project.style.display = data.selectable === false ? 'none' : '';
  if (data.selectable !== false) _loadProjectFiles(data.path);
}
...
async function _loadProjectFiles(workspace) {
    _projectRoot = workspace;
    _selectedFile = '';
    const list = _modal && _modal.querySelector('#workspace-project-files');
    const editor = _modal && _modal.querySelector('#workspace-file-editor');
    if (list) list.innerHTML = '<div class="workspace-empty">Loading project files…</div>';
    if (editor) editor.value = '';
    try {
      const res = await fetch(`${API_BASE}/api/workspace/files?workspace=${encodeURIComponent(workspace)}`, { credentials: 'same-origin' });
      if (!res.ok) throw new Error(`files failed: ${res.status}`);
      const data = await res.json();
      if (workspace !== _projectRoot || !_modal) return;
      if (!data.files || !data.files.length) {
        list.innerHTML = '<div class="workspace-empty">No safe project files</div>';
        return;
      }
      list.innerHTML = data.files.map((file) =>
        `<button type="button" class="workspace-file-item" data-file="${encodeURIComponent(file.path)}">${uiModule.esc(file.path)}</button>`
      ).join('') + (data.truncated ? '<div class="workspace-empty">File list truncated</div>' : '');
      list.querySelectorAll('.workspace-file-item').forEach((button) => {
        button.addEventListener('click', () => _openProjectFile(decodeURIComponent(button.dataset.file)));
      });
    } catch (_) {
      if (list) list.innerHTML = '<div class="workspace-empty">Could not load project files</div>';
    }
  }

async function _openProjectFile(path) {
    _selectedFile = path;
    const editor = _modal.querySelector('#workspace-file-editor');
    const status = _modal.querySelector('#workspace-file-status');
    if (status) status.textContent = 'Loading…';
    try {
      const query = `workspace=${encodeURIComponent(_projectRoot)}&path=${encodeURIComponent(path)}`;
      const res = await fetch(`${API_BASE}/api/workspace/file?${query}`, { credentials: 'same-origin' });
...
async function _saveProjectFile() {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/workspace/file`, {
        ...
        body: JSON.stringify({ workspace: _projectRoot, path: _selectedFile, content: editor.value }),
      });
...
function _previewProject() {
    ...
    const previewPath = /\.(?:html?)$/i.test(_selectedFile) ? _selectedFile : 'index.html';
    const query = `workspace=${encodeURIComponent(_projectRoot)}&path=${encodeURIComponent(previewPath)}`;
    window.open(`${API_BASE}/api/workspace/preview?${query}`, '_blank', 'noopener');
  }

function _downloadProject() {
    if (!_projectRoot) return;
    const link = document.createElement('a');
    link.href = `${API_BASE}/api/workspace/download?workspace=${encodeURIComponent(_projectRoot)}`;
    link.download = 'project-workspace.zip';
...
function _getModal() {
  ...
  _modal.innerHTML = `
    <div class="modal-content">
      ...
      <section class="workspace-project" id="workspace-project" style="display:none;">
        <div class="workspace-project-actions">
          <button type="button" class="confirm-btn confirm-btn-secondary" id="workspace-preview">Preview</button>
          <button type="button" class="confirm-btn confirm-btn-secondary" id="workspace-download">Download source</button>
        </div>
        <div class="workspace-project-editor">
          <div class="workspace-file-list" id="workspace-project-files"></div>
          <div class="workspace-editor-pane">
            <div class="workspace-editor-heading"><span id="workspace-file-status">Select a text file</span><button type="button" class="confirm-btn confirm-btn-primary" id="workspace-save">Save</button></div>
            <textarea id="workspace-file-editor" spellcheck="false" placeholder="Select a text file to edit"></textarea>
          </div>
        </div>
      </section>
  ...
  _modal.querySelector('#workspace-preview').addEventListener('click', _previewProject);
  _modal.querySelector('#workspace-download').addEventListener('click', _downloadProject);
  _modal.querySelector('#workspace-save').addEventListener('click', _saveProjectFile);
