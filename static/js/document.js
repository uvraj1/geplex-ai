...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  const _EMAIL_LOCAL_DRAFT_PREFIX = 'geplex.email.replyDraft.v1:';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__geplexActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'geplex-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'geplex-doc-minimized-' + sessionId;
  ...
  // valid. The rich body's HTML is read separately on send (bgplx_html).
  ...
  let _geplexAttachMenu = null;

  function _closeGepLexAttachMenu() {
    if (_geplexAttachMenu) {
      _geplexAttachMenu.remove();
      _geplexAttachMenu = null;
    }
  ...
  function _attachMenuOutsideClick(e) {
    if (_geplexAttachMenu && !_geplexAttachMenu.contains(e.target)) _closeGepLexAttachMenu();
  }

  function _attachMenuEscape(e) {
    if (e.key !== 'Escape') return;
    _closeGepLexAttachMenu();
  }

  function _positionGepLexAttachMenu(anchor, menu) {
  ...
  function _geplexAttachLabel(item, kind) {
    if (kind === 'gallery') {
  ...
  async function _stageGepLexAttachment(kind, id) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-geplex`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function _stageGepLexZip(items) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-geplex-zip`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  function _afterGepLexAttachmentsAdded(count, label) {
    _renderComposeAttachments();
    clearTimeout(_autoSaveDebounce);
    _autoSaveDebounce = setTimeout(() => { saveDocument({ silent: true }); }, 800);
  ...
  async function _attachGepLexItem(kind, id, label, opts = {}) {
    try {
      const data = await _stageGepLexAttachment(kind, id);
      if (!data) return;
      _afterGepLexAttachmentsAdded(1, label || data.filename);
      if (!opts.keepOpen) _closeGepLexAttachMenu();
    } catch (err) {
      console.error('Failed to attach GepLex item:', err);
      if (uiModule) uiModule.showError('Failed to attach from GepLex');
    }
  }

  function _selectedGepLexAttachRows(menu) {
    return Array.from(menu?.querySelectorAll?.('.email-geplex-attach-row.is-selected') || []);
  }

  function _syncGepLexAttachSelection(menu) {
    const selected = _selectedGepLexAttachRows(menu);
    const bar = menu?.querySelector?.('.email-geplex-attach-actions');
    const count = menu?.querySelector?.('.email-geplex-attach-count');
    const attachBtn = menu?.querySelector?.('.email-geplex-attach-selected');
  ...
  async function _attachSelectedGepLexItems(menu) {
    const rows = _selectedGepLexAttachRows(menu);
    if (!rows.length) return;
    const btn = menu.querySelector('.email-geplex-attach-selected');
    if (btn) {
      btn.disabled = true;
    ...
    try {
      ...
      if (zip) {
        await _stageGepLexZip(items);
        added = 1;
      } else {
        for (const item of items) {
          await _stageGepLexAttachment(item.kind, item.id);
          added += 1;
        }
      }
      _afterGepLexAttachmentsAdded(added, zip ? 'geplex-attachments.zip' : undefined);
      _closeGepLexAttachMenu();
    } catch (err) {
      console.error('Failed to attach selected GepLex items:', err);
      if (uiModule) uiModule.showError(added ? `Attached ${added}, then failed` : 'Failed to attach from GepLex');
      _renderComposeAttachments();
    } finally {
      if (btn) {
  ...
  async function _loadGepLexAttachItems(menu, kind) {
    const list = menu.querySelector('.email-geplex-attach-list');
    if (!list) return;
    menu.dataset.odyAttachKind = kind;
    list.replaceChildren(spinnerModule.createLoadingRow('Loading…', 14));
    menu.querySelectorAll('[data-gplx-attach-kind]').forEach(btn => {
      btn.classList.toggle('active', btn.dataset.odyAttachKind === kind);
    });
    const q = (menu.querySelector('.email-geplex-attach-search')?.value || '').trim();
    try {
      ...
      if (!items.length) {
        list.innerHTML = `<div class="email-geplex-attach-empty">${q ? 'No matches' : `No ${kind === 'gallery' ? 'images' : 'documents'}`}</div>`;
        _syncGepLexAttachSelection(menu);
        return;
      }
      list.innerHTML = '';
      for (const item of items) {
        const label = _geplexAttachLabel(item, kind);
        const row = document.createElement('button');
        row.type = 'button';
        row.className = `email-geplex-attach-row ${kind === 'gallery' ? 'is-gallery' : ''}`;
        row.dataset.id = item.id || '';
        row.dataset.kind = kind;
        if (kind === 'gallery') {
          const src = item.url ? `${API_BASE}${item.url}` : '';
          row.innerHTML = `
            <span class="email-geplex-attach-dot" aria-hidden="true"></span>
            <span class="email-geplex-attach-thumb">${src ? `<img src="${_escHtml(src)}" alt="">` : ''}</span>
            <span class="email-geplex-attach-main">
              <span class="email-geplex-attach-title">${_escHtml(label)}</span>
              <span class="email-geplex-attach-meta">${_escHtml(item.filename || 'image')}</span>
            </span>
          `;
        } else {
          row.innerHTML = `
            <span class="email-geplex-attach-dot" aria-hidden="true"></span>
            <span class="email-geplex-attach-icon">${langIcon(item.language || 'text', 14, { style: 'opacity:0.8;' })}</span>
            <span class="email-geplex-attach-main">
              <span class="email-geplex-attach-title">${_escHtml(label)}</span>
              <span class="email-geplex-attach-meta">${_escHtml(item.language || 'text')}</span>
            </span>
          `;
        }
        ...
        row.addEventListener('dblclick', () => _attachGepLexItem(kind, item.id, label, { keepOpen: false }));
        list.appendChild(row);
      }
      _syncGepLexAttachSelection(menu);
    } catch (err) {
      console.error('Failed to load GepLex attach items:', err);
      list.innerHTML = '<div class="email-geplex-attach-empty">Could not load</div>';
    }
  }
  ...
  function _showComposeAttachMenu(anchor) {
    ...
    menu.className = 'email-geplex-attach-menu';
    menu.innerHTML = `
      <button type="button" class="email-geplex-attach-local">
      ...
      <div class="email-geplex-attach-tabs">
        <button type="button" data-gplx-attach-kind="document" class="active">
        ...
        <button type="button" data-gplx-attach-kind="gallery">
      ...
      <label class="email-geplex-attach-search-wrap">
        ...
        <input type="search" class="email-geplex-attach-search" placeholder="Search attachments">
      </label>
      <div class="email-geplex-attach-list"></div>
      <div class="email-geplex-attach-actions">
        <span class="email-geplex-attach-count"></span>
        <button type="button" class="email-geplex-attach-selected" disabled>
    ...
    _geplexAttachMenu = menu;
    _positionGepLexAttachMenu(anchor, menu);
    menu.querySelector('.email-geplex-attach-local')?.addEventListener('click', () => {
      _closeGepLexAttachMenu();
      document.getElementById('doc-email-file-input')?.click();
    });
    menu.querySelectorAll('[data-gplx-attach-kind]').forEach(btn => {
      btn.addEventListener('click', () => _loadGepLexAttachItems(menu, btn.dataset.odyAttachKind));
    });
    let attachSearchTimer = null;
    menu.querySelector('.email-geplex-attach-search')?.addEventListener('input', () => {
      clearTimeout(attachSearchTimer);
      attachSearchTimer = setTimeout(() => {
        _loadGepLexAttachItems(menu, menu.dataset.odyAttachKind || 'document');
      }, 220);
    });
    menu.querySelector('.email-geplex-attach-selected')?.addEventListener('click', () => _attachSelectedGepLexItems(menu));
    setTimeout(() => {
      document.addEventListener('click', _attachMenuOutsideClick, true);
      document.addEventListener('keydown', _attachMenuEscape, true);
    }, 0);
    _loadGepLexAttachItems(menu, 'document');
  }
  ...
  async function _sendEmail() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/email/send`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          to, cc: cc || null, bcc: bcc || null, subject, body, bgplx_html: bodyHtml,
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__geplexActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const _AI_REPLY_CONTEXT_STORE_PREFIX = 'geplex:email-ai-reply-context:v1:';
  ...
  async function _aiReply(opts = {}) {
    ...
    const sourceUid = document.getElementById('doc-email-source-uid')?.value?.trim() || '';
    const sourceFolder = document.getElementById('doc-email-source-folder')?.value?.trim() || 'INBOX';
    const sourceAccountId = docs.get(activeDocId)?.sourceEmailAccountId || window.__geplexActiveEmailAccount || '';
  ...
  export function openPanel() {
    ...
    pane.innerHTML = `
      ...
      <div id="doc-email-actions" class="doc-email-actions" style="display:none">
        ...
        <div class="email-send-split">
          <button type="button" id="doc-email-send-btn" class="email-send-btn email-send-main" title="Send email (Ctrl+Enter)" onpointerdown="window.geplexEmailSendIntent&&window.geplexEmailSendIntent(event)" onmousedown="window.geplexEmailSendIntent&&window.geplexEmailSendIntent(event)" onclick="window.geplexEmailSendIntent&&window.geplexEmailSendIntent(event)"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>Send</button>
          <button type="button" id="doc-email-send-caret" class="email-send-btn email-send-caret" title="More send options" aria-haspopup="true" aria-expanded="false" onpointerdown="window.geplexEmailCaretIntent&&window.geplexEmailCaretIntent(event)" onmousedown="window.geplexEmailCaretIntent&&window.geplexEmailCaretIntent(event)"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9"/></svg></button>
    ...
    (function _initLangPicker() {
      ...
      // Menu is bgplx-mounted so position:fixed coords work cleanly.
      document.body.appendChild(menu);
    ...
    const handleSendIntent = (e) => {
      if (e && e.__geplexEmailSendHandled) return;
      ...
      if (e) {
        e.preventDefault();
        e.stopPropagation();
        e.__geplexEmailSendHandled = true;
      }
    ...
    window.geplexEmailSendIntent = handleSendIntent;
    ...
    const handleCaretIntent = (e) => {
      if (e && e.__geplexEmailCaretHandled) return;
      const now = Date.now();
      if (e && e.type === 'click' && now - lastCaretToggleAt < 350) {
        e.preventDefault();
        e.stopPropagation();
        e.__geplexEmailCaretHandled = true;
        return;
      }
      const rawTarget = e && e.target;
      const target = rawTarget && rawTarget.nodeType === Node.TEXT_NODE ? rawTarget.parentElement : rawTarget;
    ...
    window.geplexEmailCaretIntent = handleCaretIntent;
    ...
    let _fontIdx = parseInt(localStorage.getItem('geplex-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('geplex-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'geplex-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('geplex-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('geplex-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('geplex-suggestions-' + docId);
      if (!raw) return;
