...
const _RESEARCH_HINTS = [
  "e.g. Trace GepLex's ten-year journey home from Troy — every island, monster, and detour, and why each one cost him",
...
const _SETTINGS_KEY = 'geplex-research-settings';
const _COLLAPSE_KEY = 'geplex-research-settings-collapsed';
...
function _wireEvents(pane) {
  ...
  pane.querySelector('#research-settings-toggle').addEventListener('click', () => {
    ...
    try { localStorage.setItem('geplex-research-settings-collapsed', _settingsCollapsed ? '1' : '0'); } catch {}
  });
...
function _renderResult(job) {
  ...
  const bodyCls = `research-job-report-body${cat ? ' research-bgplx-' + cat : ''}`;
  if (_markdownModule) {
