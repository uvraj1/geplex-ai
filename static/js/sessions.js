...
let _sortMode = Storage.get('geplex-session-sort') || 'active'; // default to last active
const DATE_SECTION_COLLAPSE_KEY = 'gplx-session-date-section-collapsed';
let _autoCreateInProgress = false; // guard against recursive auto-create
const _INCOGNITO_SESSIONS_KEY = 'gplx-incognito-sessions'; // sessionStorage key for incognito session IDs
...
function _shouldPreserveStartupComposer(msgInput) {
  ...
  if (window.__geplexComposerUserEdited) return true;
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'GepLex Chat';
...
const FOLDER_STATE_KEY = 'geplex-folder-state';
const FOLDER_ORDER_KEY = 'geplex-folder-order';
...
function _showSwipeHint(list) {
  if ('ontouchstart' in window && !localStorage.getItem('gplx-swipe-hint-shown')) {
    const firstItem = list.querySelector('.session-item');
    if (firstItem) {
      localStorage.setItem('gplx-swipe-hint-shown', '1');
...
export async function loadSessions() {
  try {
    ...
    const prefetched = sessionStorage.getItem('gplx-prefetch-sessions');
    let fetched;
    if (prefetched) {
      sessionStorage.removeItem('gplx-prefetch-sessions');
      ...
      if (!res.ok) {
        if (res.status === 404) {
          fetched = Storage.get('geplex-cached-sessions') || [];
        } else {
          let detail = '';
          try {
        ...
        if (Array.isArray(fetched)) {
          Storage.set('geplex-cached-sessions', fetched);
        }
      }
    }
    if (!Array.isArray(fetched)) {
    ...
    const _isFirstLoad = !sessionStorage.getItem('gplx-session-active');
    ...
    // most-recent).
    if (_isFirstLoad) sessionStorage.setItem('gplx-session-active', '1');
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  ...
  try {
    ...
    currentSessionId = id;
    try { window.__geplexLastSelectedSessionId = id; } catch (_) {}
    ...
    // URL hash — the user complained that coming back to GepLex kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'GepLex Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('geplex-doc-open-' + id) === '1';
...
async function _getPreferredDefaultChat() {
  let dc = null;
  try {
    dc = window.__geplexDefaultChat || null;
  } catch (_) {}
  if (!dc || !dc.endpoint_url || !dc.model) {
    try {
...
export function createDirectChat(url, modelId, endpointId, opts = {}) {
  ...
  currentSessionId = null;
  try { window.__geplexLastSelectedSessionId = ''; } catch (_) {}
  Storage.remove('lastSessionId');
  history.replaceState(null, '', window.location.pathname);
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('geplex-session-sort', mode);
  else Storage.remove('geplex-session-sort');
  renderSessionList();
}
