...
export function initSidebarLayout(Storage, opts) {
  ...
  const SIDEBAR_MODE_KEY = 'geplex-sidebar-mode';

  function _setSidebarModeClasses(mode) {
    document.documentElement.classList.remove('gplx-mobile-startup-sidebar-hidden');
    document.documentElement.classList.toggle('gplx-sidebar-mini', mode === 'mini');
    document.documentElement.classList.toggle('gplx-sidebar-off', mode === 'off');
  }
  ...
  function _applyStoredSidebarMode() {
    ...
    if (!sidebar) return;
    // Start every page with the full sidebar closed. The hamburger is the
    // explicit user action that opens it; this avoids restoring a stale
    // "full" preference before the user asks for the navigation panel.
  ...
  if (hamburgerBtn) {
    hamburgerBtn.addEventListener('click', (e) => {
      ...
      // Desktop: full sidebar ↔ completely hidden — clean 1-click toggle
      if (isSidebarVisible) {
        sidebar.classList.add('hidden');
        if (iconRail) iconRail.classList.add('rail-hidden');
        _saveSidebarMode('off');
  ...
  document.addEventListener('click', (e) => {
    ...
    // overlay — they're bgplx-level elements logically tied to a sidebar
